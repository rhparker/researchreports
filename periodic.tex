...
We also have the following piecewise bounds on $L_2$.

\begin{align*}
|L_2(\lambda)(a,b,c,d)_i^-| &\leq C (|a| + |b| + e^{\nu(\lambda)X_{i-1}}|c_{i-1}^-| + |\lambda|^2 |d| ) \\
...
&+ \Phi^s_+(X_i, 0; \lambda)b_i^+ - \Phi^u_-(-X_i, 0; \lambda)b_{i-1}^- \\
&+ \int_0^{-X_i} \Phi^u_+(-X_i, y; \lambda) [ G_i^+(y) W_i^+(y) + d_i \lambda^2 \tilde{H}(y) ] dy \\
&- \int_0^{X_i} \Phi^s_-(X_i, y; \lambda) [ G_{i-1}^-(y) W_{i-1}^-(y) + d_{i-1} \lambda^2 \tilde{H}(y) ] dy \Big)
\end{align*}
...
For the bound on $L_3$, we have seen most of the terms before. For the bound on the integral term involving $\tilde{H}$, we use the $\tilde{\alpha}$ trick to get a better bound for the term involving $\tilde{H}$. 
...
+ e^{-\alpha X_i}|b| + p_4(X_i; \lambda)|c_i| + ||G|| ||W|| + e^{-\tilde{\alpha} X_i} |\lambda^2| |d| )
\]

where

\[
p_4(X_i; \lambda) = p_2(X_i; \lambda) + p_2(X_i; \lambda)
\]

To get the final bound for $L_3$, we plug in the $W_1$ for $W$ and use the bound \eqref{W1bound}. Thus we have

\begin{align*}
|L_3&(\lambda)(a, b, c, d)_i| \\
&\leq C \Big( p_1(X_i; \lambda)|a_i|
+ e^{-\alpha X_i}|b| + p_4(X_i; \lambda)|c_i| + ||G||\:||W_1(\lambda)(a,b,c,d)|| + e^{-\tilde{\alpha} X_i} |\lambda^2| |d| \Big) \\
\end{align*}

Collecting terms and eliminating higher order terms, we attain the final bound for $L_3$.
...
&+ ( e^{\nu(\lambda)X_i} + p_4(X_i; \lambda) + e^{\nu(\lambda)X_i} ||G|| ) |c_i| \\
&+ ( e^{\nu(\lambda)X_{i-1}} + p_4(X_{i-1}; \lambda) + e^{\nu(\lambda)X_{i-1}} ||G||) |c_{i-1}| \\ 
...
||G||(e^{-\alpha X_{i-1}} + ||G||) ) |b| \\
&+ (p_1(X_i; \lambda) + ||G|| + 1)(p_4(X_i; \lambda) + e^{\nu(\lambda)X_i} ||G||) |c_i| \\
