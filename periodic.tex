...
In this section we will look at multipulse solutions to KdV5 in the case where we have periodic boundary conditions. Note that we cannot use an exponential weight in this case since our domain is of finite length. A periodic $n$-pulse is parameterized by the $n$ lengths $X_0, \dots, X_{n-1}$, where in general we will want $X_{n-1}$ to be larger than the other $X_i$. (Since we have a periodic domain, we start with $X_0$ for convenience so that all the subscripts will be $\mod n$.) A diagram of a 2-pulse is shown below.

\begin{figure}[H]
\includegraphics[width=8.5cm]{dpimage}
\end{figure}

Let $X_m = \min \{ X_0, \dots, X_{n-1} \}$ and $X_M = \max \{ X_0, \dots, X_{n-1} \}$. The way we have set this up, we have $X_M = X_{n-1}$, but it does not actually matter since the domain is periodic.\\
...
Using the parameterizations of the stable and unstable manifolds, we write the solution $U$ piecewise (in $2n$ pieces, $i = 0, \dots, n-1$) as
...
for $i = 0, \dots, {n-1}$. The subscripts are $\mod n$, so that, in particular, $U_n^- = U_0^-$.\\
...
For any $X_i$ and $\beta_i^\pm$ chosen as in the previous lemma, there is a unique pair of initial conditions $(a_i^+, a_i^-) \in E_0^s \times E_0^u$ such that $U_i^+(X_i) - U_{i+1}^-(-X_i) = 0$. $(a_i^+, a_i^-)$ depends smoothly on $(\beta_i^+, \beta_{i+1}^-)$, and we have the following estimate, which is independent of the $\beta_i^\pm$
...
U_{i+1}^-(-X_i) &= Q^-(0, \beta_{i_1}^-)(-X_i) + P^s_-(-X_i; \beta_{i+1}^-) a_i^- \\
...
&- \int_0^{-X_i} \Phi_-^u(-X_i, y; \beta_{i+1}^-) G_{i+1}^-(y, V_{i+1}^-(a_i^-, \beta_i^-)(y),\beta_{i+1}^-)dy 
\end{align*}
...
We will do each of these in turn. In the next lemma, we solve for $(\beta_i^+, \beta_i^i)$ for $i = 0, \dots, n-1$. We do this for each pair separately.

% lemma : solve for the \beta_i^\pm

\begin{lemma}\label{solveforbeta}
There exist $\beta_i^\pm$ such that $P_{Y^\pm}(U_i^+(0) - U_i^-(0)) = 0$ for $i = 0, \dots, n-1$. We have the estimate
...
Using this together with the estimate on $\beta_i^\pm$ from Lemma \ref{solveforbeta} gives us for $x \geq 0$
...
&= \langle \Psi(0), \Phi(0, X_i) Q^-(0, \beta_i^-)(-X_i) \rangle + \mathcal{O}(e^{-3 \alpha X_i} + e^{-2 \alpha X_i} (e^{-2 \alpha X_i} + e^{-2 \alpha X_{i-1}})) \\
...
\begin{align*}
 \langle \Psi(0), \Phi^s_-(0, -X_{i-1}, \beta_i^-) a_{i-1}^- \rangle 
&= \langle \Psi(-X_{i-1}), Q(X_{i-1}) \rangle + \mathcal{O}(e^{-3 \alpha X_{i-1}} + e^{-2 \alpha (X_i + X_{i-1})}) 
\end{align*}
...
Combining these, and noting the order $e^{-2 \alpha (X_i + X_{i-1})}$ term is subsumed into the order $e^{-3 \alpha X_i}$ and order $e^{-3 \alpha X_{i-1}}$ terms, we have
...
In the next lemma, we rewrite the system of equations we are looking to solve using the above expressions for the appropriate inner products. The idea is that we want to get a decay rate for the remainder terms $R_i$ which does not depend on the individual $X_i$. The way we will accomplish this is have it depend on the smallest of the $X_i$.
...
and we recall that $X_m = \min \{ X_0, \dots, X_{n-1} \}$. Thus we will always have $a_i = 1$ for some $i$.
...
In the next lemma, we show we don't need to solve the last one of these equations in the Hamiltonian case.

% lemma : get rid of equations

\begin{lemma}\label{reducedsystemtosolve}
If the system is Hamiltonian, a periodic double pulse solution exists if and only if for $i = 1, \dots, n-1$
...
\begin{proof} 
If the system is Hamiltonian, the energy $H$ is conserved, thus all solutions must exist within level sets of $H$. Recall that $\Psi(0) = \nabla H(Q(0)) \neq 0$. Thus the energy $H$ must change along any jump in the direction of $\Psi(0)$. In Lemma \ref{solvefora}, we matched the tails of the pieces. Assume we have matched $U_i^-(0) = U_i^+(0)$ for all $i$ except $i = i_0$. In Lemma \ref{solveforbeta}, we matched the $Y^\pm$ components of $U_{i_0}^-(0) = U_{i_0}^+(0)$. All that remains is to match the $\R \Psi(0)$ component of $U_{i_0}^-(0) = U_{i_0}^+(0)$. But this is automatically 0 since the two pieces involved must have the same energy, and if this were nonzero there would be a change of energy at this point. Thus we can eliminate one of the equations from the previous lemma. Since it does not matter which we eliminate, we might as well eliminate the first one.\\
\end{proof}
\end{lemma}
...
However, we have two more unknowns than equations, which is not good. Luckily there is a way out. Recall that by the way we set things up, $a_i = 1$ for some $i$. Since we are on a periodic domain, we can WLOG ``rotate'' things so that $a_0 = 1$. We now have a system of equations $G: \R^{n-1} \rightarrow \R^{n-2}$ defined by 
...
and $m \in \N$. Noting that $(\beta / \alpha) \log r_m = -m \pi$, so that $\sin \left(-\frac{\beta}{\alpha} \log r_m \right) = \sin(m \pi) = 0$, the equations for $G$ become
...
This is the system we are looking to solve. The only dependence on $r_m$ is via the remainder term on the RHS, and we know that $\tilde{R}_i(a, r_m) = \mathcal{O}(r_m^{\tilde{\gamma}})$ from Lemma \ref{systemtosolve}. By continuity of the remainder term in $r$, we can send $m \rightarrow \infty$ (thus $r_m \rightarrow 0$) to get

\begin{align}
G_1(a_1, \dots, a_{n-1}, 0) &= -a_1 \sin \left( - \frac{\beta}{\alpha} \log a_1 \right) \\
G_i(a_1, \dots, a_{n-1}, 0) &= a_{i-1} \sin \left(-\frac{\beta}{\alpha} \log a_{i-1} \right) - a_i \sin \left(-\frac{\beta}{\alpha} \log a_i \right) && i = 2, \dots, n-1 \\
\end{align}

Now, for $i = 1, \dots, n-1$, let

\begin{equation}
a_i^0 = e^{-\frac{\pi \alpha}{\beta} k_i^0 }
\end{equation}

for $k_i^0 \in \N_0$. Then we have $G(a_1^0, \dots, a_{n-1}^0, 0) = 0$. We are now in place to use the implicit function theorem to solve for $a_1, \dots, a_{n-1}$ in terms of $r$ near $(a_1^0, \dots, a_{n-1}^0, 0)$. To verify that we can do this, we need to take the partial derivatives of the component functions $G_i$ with respect to $a_i$.\\ 
...
where we took the limit as $r \rightarrow 0$ as we did above. The Jacobian matrix of $G$ with respect to $(a_1, \dots, a_{n-1})$ at $(a_1^0, \dots, a_{n-1}^0, 0)$ is 

\begin{equation}
DG_{(a_1, \dots, a_{n-1})}(a_1^0, \dots, a_{n-1}^0, 0) 
= \dfrac{\beta}{\alpha}
\begin{pmatrix}
(-1)^{k_1^0} & & & \\
(-1)^{k_1^0 + 1} & (-1)^{k_2^0} \\
...
\[
G(a_1(r), \dots, a_{n-1}(r), r) = 0
\]

Now take $m \in \N$ so that $r_m < r_0$. Then

\[
G(a_1(r_m), \dots, a_{n-1}(r_m), r_m) = 0
\]
...
\begin{lemma}
For sufficiently large $m \in \N$, there exists a countable family of lengths $(X_0(m), X_1(k_1^0; m), \dots, X_{n-1}(k_{n-1}^0; m))$ with $k_i^0 \in \N_0$ such that
...
For the remaining $X_i$, we have $a_i(r_m) \approx e^{-\frac{\pi \alpha}{\beta} k_i^0 }$. Solving the expression for $a_i$ in terms of $X_i$ and using this approximation, we have for $i = 1, \dots, n-1$
...
We have successfully constructed a periodic multipulse. We now combine these results in the following theorem
