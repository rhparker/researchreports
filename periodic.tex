...
Since we are only intererested in the existence problem here, we can use the 4th order (integrated) equation in the traveling frame
...
Suppose $U = 0$ is an equilibrium for this equation (i.e. $F(0) = 0$), and suppose $Q(x)$ is a homoclinic orbit connecting this equilibrium to itself. In addition, we assume the following

\begin{hypothesis}\label{assumptions}
\begin{enumerate}
	\item $U = 0$ is a hyperbolic equilibrium.
	\item The spectrum of $D_U F(0)$ is $\pm \alpha \pm i \beta$, $\alpha, \beta > 0$.
...
If \eqref{4thordersystem} is Hamiltonian, then
...
thus $\nabla H(Q(x))$ is a solution to the adjoint variational equation. Since $\nabla H$ is continuous and $Q(x)$ is bounded (even better, it decays exponentially to 0), $\nabla H(Q(x))$. By the nongeneracy condition, there is a unique such solution, so we conclude that $\Psi(x) = \nabla H(Q(x))$.

\end{proof}
\end{lemma}

Next, we paramaterize the stable and unstable manifolds near $Q(0)$. To do that we first write
...
Since we are assuming $D_U F(0)$ has two stable and two unstable directions, we have 

\[
\R^4 = \R Q'(0) \oplus Y^+ \oplus Y^- \oplus Z
\]
 
We have also shown before that $Z$ is perpendicular to the other three. \\

We now write $W^u(0)$ and $W^s(0)$ as graphs over their own tangent spaces near $Q(0)$. Following San97, we can paramaterize the unstable and stable manifolds near $Q(0)$ by the smooth functions
...
Using the paramaterizations of the stable and unstable manifolds, we write $U$ piecewise (in four pieces, $i = 1, 2$) as
...
\begin{equation}
V' = D_U F(Q^\pm(0, \beta^\pm)(x)) V
\end{equation}

We can decompose these evolution operators via an exponential dichotomy.
...
\text{ran } P^u_+(0; \beta^+) &= Z \oplus Y^- \\
\text{ran } P^s_-(0; \beta^-) &= Z \oplus Y^+
\end{align*}
...
Now, we rewrite our problem $U'(x) = F(U)$ in piecewise fashion in such a way as to exploit what we did above. We want to solve piecewise

\begin{align*}
(U_i^\pm(x))' &= (Q^\pm(0, \beta_i^\pm)(x))' + (V_i^\pm(x))' &= F(Q^\pm(0, \beta_i^\pm)(x) + V_i^\pm(x) ) \\
\end{align*}
...
where $G_i^\pm(\beta_i^\pm, V_i^\pm)(x) = \mathcal{O}(|V_i^\pm|^2)$, independent of the parameters $\beta_i^\pm$. Derivatives of $G_i^\pm$ with respect to the parameters $\beta_i^\pm$ are the same order in $V_i^\pm$.\\
...
Note that we do not have initial conditions $b_i^\pm$ for the other side of the dichotomy. Since these ICs would live in $Y^\pm$, we have incorporated them into the $\beta_i^\pm$.\\
...
Before we do that, we will need to make a smooth change of coordinates near $Q(0)$. We will do that using a variation of the ``flow-box'' method. What we want to do is straighten out the stable and unstable manifolds near $Q(0)$ so that their nonintersecting directions lie along $Y^+$ and $Y^-$.
...
In other words, $H(y; \beta^-, \beta^+, \gamma)$ is a tracjectory of $U' = F(U)$ with initial condition $Q(0) + Q^-(\beta^-,0) + Q^-(\beta^+,0) + \gamma \Psi(0)$.\\ 
...
We would like to get an estimate on this. By the inverse function theorem, $D H^{-1}$ is bounded in a neighborood of $(0, 0)$, thus we have 
...
Finally, we need to look at the projection on the adjoint solution $\Psi(0)$. To do this, we will show that this condition is satisfied for certain values of the lengths $X_i$. This makes sense since we only have existence of (nonperiodic) multipulses for certain $X_i$.\\
...
Before we continue, we make the following hypothesis (which we should be able to show)

\begin{hypothesis}
\[
|(\Phi_\pm(0, X; \beta^\pm) - \Phi(0, X))|
\leq C |\beta^\pm| e^{-\alpha X}
\]
\end{hypothesis}

Together with the previous lemma, this gives us

\[
|(\Phi_\pm(0, X_i; \beta_i^\pm) - \Phi(0, X_i))|
\leq C e^{-\alpha X_i} (e^{-\alpha X_1} + e^{-\alpha X_2})
\]
...
We also have the following estimate from Hypothesis \label{p1existence}
...
Since from our change of coordinates, $Q^+(0, \beta_i^+) \in Y^+$, and this is wiped out by $P^u_+(0; \beta_i^+)$, which projects onto the unstable part of the exponential dichotomy on $\R^+$ at $x = 0$. We also used the fact that the spectral projections $P^u_0$ and $P^s_0$ commute. \\
...
The other integral term is similar, and is order $e^{-3 \alpha X_{i-1}}$\\
...
\[
R_i(a, r) = \mathcal{O}(r^{1 + \tilde{\gamma}})
\]
...
For sufficiently small $r > 0$, there is a countable family of solutions $(a_1(k_1^0, r), a_2(k_2^0, r)$ to 

\begin{align*}
a_2 \sin \left( - \frac{\beta}{\alpha} \log a_2 r \right) - a_1 \sin \left( - \frac{\beta}{\alpha} \log a_1 r \right) = \tilde{R}_1(a, r)
\end{align*}

where $k_i^0 \in \N_0$ and 

\begin{align*}
a_i(k_i^0, r) \approx e^{-\frac{\pi \alpha}{\beta} k_i^0 }
\end{align*}
...
for $k_i^0 \in \N_0$. Then we have $G(a_1^0, a_2^0, 0) = 0$. We are now in position to use the implicit function theorem (twice!). It is worth noting the difference between this and the nonperiodic case considered on p.2094 of SanStrut. In that case, the the Jacobian of the reduced system $G:\R^{n-1} \times \R \rightarrow \R$ is lower triangular, so a single application of the implicit function theorem is sufficient. Since our case is periodic, every equation in Lemma \ref{systemtosolve} involves two of the $a_i$, so we can never get this lower triangular structure. If we take the nonreduced system, the Jacobian is singular at $(a_1^0, a_2^0, 0)$, so that will not work. Of course, we suspected as much since the second equation is superfluous, hence the reduction. So let us continue.\\
...
&= \sin \left( - \frac{\beta}{\alpha} \log a_i r \right)\Big|_{a_i = a_i^0, r = 0} - a_i \cos \left( - \frac{\beta}{\alpha} \log a_i r \right)a_i \frac{\beta}{\alpha} \Big|_{a_i = a_i^0, r = 0}\\
&=(-1)^{k_i^0 + 1}\frac{\beta}{\alpha}
\end{align*}

where we took the limit as $r \rightarrow 0$ as we did above. First we solve for $a_1$. Since 

\[
\frac{\partial}{\partial a_i} G(a_1^0, a_2^0, 0) = -(-1)^{k_i^0 + 1}\frac{\beta}{\alpha} \neq 0
\]

we can use the implicit function theorem to solve for $a_1$ in terms of $(a_2, r)$ near $(a_2^0, 0)$. Thus we have near $(a_2, r) = (a_2^0, 0)$

\begin{equation}
G(a_2, r) = a_2 \sin \left( - \frac{\beta}{\alpha} \log a_2 r \right) - a_1(a_2, r) \sin \left( - \frac{\beta}{\alpha} \log a_1(a_2, r) r \right) - \tilde{R}_1(a, r)
\end{equation}

where $a_1(a_2^0, 0) = a_1^0$. From the implicit function theorem, we also have an expression for the partial derivative of $a_1$ with respect to $a_2$ at $(a_2^0, 0)$.

\begin{align*}
\frac{\partial}{\partial a_2} a_1(a_2^0, 0) 
&= -\left(\frac{\partial G}{\partial a_1}\Big|_{(a_1(a_2^0, 0), a_2^0, 0)}\right)^{-1}
\frac{\partial G}{\partial a_2}\Big|_{(a_1(a_2^0, 0), a_2^0, 0)} \\
&= -\left(-\frac{\beta}{\alpha} (-1)^{k_1^0 + 1} \right)^{-1} \frac{\beta}{\alpha} (-1)^{k_2^0 + 1} \\
&= (-1)^{k_1^0 + k_2^0}
\end{align*}

Now we use the implicit function theorem again to solve for $a_2$ in terms of $r$ near $r = 0$. First we compute the partial derivative with respect to $a_2$.

\begin{align*}
\frac{\partial}{\partial a_2} G(a_2^0, 0) &= 
\frac{\beta}{\alpha} (-1)^{k_2^0 + 1} - \frac{\partial}{\partial a_2}\left( a_1(a_2, r) \sin \left( - \frac{\beta}{\alpha} \log a_1(a_2, r) r \right)\right) \Big|_{(a_2^0, 0)} \\
&= \frac{\beta}{\alpha} (-1)^{k_2^0 + 1} - (-1)^{k_1^0 + k_2^0} \sin (\pi k_1^0) - a_1(a_2^0, 0) \frac{\partial}{\partial a_2} \sin \left( - \frac{\beta}{\alpha} \log a_1(a_2, r) r \right)\Big|_{(a_2^0, 0)} \\
&= \frac{\beta}{\alpha} (-1)^{k_2^0 + 1} - a_1^0  \cos \left(-\frac{\beta}{\alpha} a_1^0 \right) \left(-\frac{\beta}{\alpha}\right) \frac{1}{a_1^0} \frac{\partial}{\partial a_2} a_1(a_2, r) \Big|_{(a_2^0, 0)}\\
&= \frac{\beta}{\alpha} (-1)^{k_2^0 + 1} + \left(\frac{\beta}{\alpha}\right) (-1)^{k_1^0 + 1} (-1)^{k_1^0 + k_2^0} \\
&= -\frac{\beta}{\alpha} \left( (-1)^{k_2^0} +  (-1)^{k_1^0} (-1)^{k_1^0} (-1)^{k_2^0} \right) \\
&= -2 \frac{\beta}{\alpha} (-1)^{k_2^0} \neq 0
\end{align*}

Since this is nonzero, we can use the implicit function theorem a second time to solve for $a_2$ in terms of $r$ near $r = 0$. Thus near $r = 0$ we have $G(a_1(a_2(r), r), a_2(r), r) = 0$, where $a_2(0) = a_2^0$ and $a_1(a_2(0), 0) = a_1^0$. Since the join occurs when $G(a_1(a_2(r), r), a_2(r), r) = 0$, the proof is complete.
...
\begin{lemma}
For sufficiently large $X > 0$, there exist a countable family of lengths $(X_1(k_1^0, X), X_2(k_2^0, X))$ with $k_i^0 \in N_0$ such that

\[
\langle \Psi(X_i), Q(-X_i) \rangle - \langle \Psi(-X_{i-1}), Q(X_{i-1}) \rangle + R_i = 0
\]

The lengths are approximately given by

\[
X_i(k_1^0, X) \approx \frac{1}{4} \left( \frac{2 \pi}{\beta} \right) k_i^0 + X
\]

\begin{proof}
Let $r > 0$ be such that the previous lemma applies, and let $a_i(k_i^0, r)$
be as in that lemma. Then solve for $X$ in terms of $r$ to get

\[
X(r) = \frac{1}{2 \alpha}\log\left(\frac{1}{r}\right) - \frac{\phi}{2 \beta}
\]

Solving the expression for $a_i$ for $X_i$ yields

\[
X_i(k_1^0, X) = -\frac{1}{2 \alpha}\log(a_i(k_i^0, r)) + X(r)
\]

By the previous lemma, 

\begin{align*}
\log( a_i(k_i^0, r)) \approx -\frac{\pi \alpha}{\beta} k_i^0
\end{align*}

Thus we have

\[
X_i(k_1^0, X) \approx \frac{\pi}{2 \beta} k_i^0 + X(r)
\]

We can rewrite this as

\[
X_i(k_1^0, X) \approx \frac{1}{4} \left( \frac{2 \pi}{\beta} \right) k_i^0 + X(r)
\]

Since $\frac{2 \pi}{\beta}$ is the period of ``twisting'' of the stable and unstable manifolds near the equilibrium point, this agrees with our intuition of a join being possible every ``quarter-twist'' of the stable and unstable manifolds near the equilibrium point.
