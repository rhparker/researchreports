...
where $\Delta v$ and $\Delta w$ are defined in Lemma \ref{endjoins}. Similarly, we have
...
Next we note that the $c_i^\pm$ have coefficients like $e^{\pm \nu(\lambda) }X_i$. One of these is potentially bad (the one with positive exponential growth), but we do not know which one. To get around this, we will add and subtract the version with the other sign. \\
...
where $L_4(\lambda)(b, c)$ is the rest of the terms that don't get eliminated outright by the projections. Note that we have written $L_4$ entirely in terms of $f(X_i; \lambda) c_i^+$ and the like, which is necessary in order to perform the inversion. \\

For the bound on $L_4$, we need to get bounds on the integral terms involving $W$ in a similar fashion to what we did before. Let us find a bound for the center integral involving $W$ on the ``minus'' piece.
...
&\leq C ||G|| ( |b| + e^{\nu(\lambda)X_{i-1}}|c_{i-1}| + (|\lambda|^2 + |D_{i-1}|)|d| ) dy \\
&\leq C ||G|| ( |b| + f^+(X_{i-1}; \lambda) |f(X_{i-1}; \lambda)c_{i-1}| + (|\lambda|^2 + |D_{i-1}|)|d| ) dy
\end{align*}

where in the antipenultimate line we will take $\lambda$ sufficiently small so that $2 |\nu(\lambda)| < \alpha$, and we changed the sign on $\nu(\lambda)$ so that this holds regardless of the sign of $\nu(\lambda)$. \\
...
&\leq C ||G|| ( |b| + f^-(X_i; \lambda) |f(X_i; \lambda)c_i| + (|\lambda|^2 + |D_i|)|d| ) dy
\end{align*}
...
&+ ||G|| ( |b| + f^+(X_{i-1}; \lambda) |f(X_{i-1}; \lambda)c_{i-1}| + (|\lambda|^2 + |D_{i-1}|)|d| ) \\
&+ ||G|| ( |b| + f^-(X_i; \lambda) |f(X_i; \lambda)c_i| + (|\lambda|^2 + |D_i|)|d| + |\lambda|^2|d|)
\Big)\\
&\leq C \Big( e^{-\alpha X_i} |a_i^+| +  e^{-\alpha X_{i-1}} |a_{i-1}^-| + (p_5(\lambda) + ||G||) |b_i| \\
&+ (f^+(T; \lambda)( p_2(X_{i-1}; \lambda) + ||G||) + f^-(X_{i-1}; \lambda)) |f(X_{i-1}; \lambda) c_{i-1}^-| \\
&+ (f^-(X_i; \lambda)( p_2(X_i; \lambda) + ||G||) + f^+(X_i; \lambda)) | f(X_i; \lambda) c_i^+| \\
&+ ||G|| |D||d| + |\lambda|^2|d|
\Big)
\end{align*}

where $p_2(X; \lambda)$ is defined in Lemma \ref{endjoins}. Luckily, $p_2(X; \lambda)$ does not enter into the final bound, since we do not have a decay rate for it, but we know that its limit is 0 as $|X| \rightarrow \infty$.\\

To finish the bound, we need to plug in $A_1$ for $|a|$

\begin{align*}
|L_4&(\lambda)_i(b, c, d)|\\ 
&\leq C \Big(e^{-\alpha X_i} |A_1(\lambda)_i(b, c, d)| +  e^{-\alpha X_{i-1}} |A_1(\lambda)_{i-1}(b, c, d)| \\
&+ (p_5(\lambda) + ||G||) |b_i| \\
&+ (f^+(T; \lambda)( p_2(X_{i-1}; \lambda) + ||G||) + f^-(X_{i-1}; \lambda)) |f(X_{i-1}; \lambda) c_{i-1}^-| \\
&+ (f^-(X_i; \lambda)( p_2(X_i; \lambda) + ||G||) + f^+(X_i; \lambda)) | f(X_i; \lambda) c_i^+| \\
&+ ||G|| |D||d| + |\lambda|^2|d| \Big) \\
...
Simplifying this, and discarding higher order terms, this becomes

\begin{align*}
|L_4&(\lambda)_i(b, c, d)|\\ 
&\leq C \Big( ( e^{-\alpha X_i} + e^{-\alpha X_{i-1}} + p_5(\lambda) + ||G||)|b| \\
&+ (f^+(T; \lambda)( p_2(X_{i-1}; \lambda) + ||G||) + f^-(X_{i-1}; \lambda) + e^{-\alpha X_{i-1}}) |f(X_{i-1}; \lambda) c_{i-1}^-| \\
&+ (f^-(X_i; \lambda)( p_2(X_i; \lambda) + ||G||) + f^+(X_i; \lambda) + e^{-\alpha X_i}) | f(X_i; \lambda) c_i^+| \\
&+ (||G|| + e^{-\alpha X_i})|D_i||d| + (||G|| + e^{-\alpha X_{i-1}})|D_{i-1}||d| \\
&+ |\lambda|^2|d| \Big) \\
\end{align*}
