...
\end{equation}
where $\gamma > 0$, $s_0 > 0$, and $\phi$ are constants.
\begin{proof}
BASED ON ASSUMPTIONS, this follows from Lemma 6.1 in San98. In our case, we do not have a parameter $\mu$, so the $\mu$-dependent terms are constant. 
\end{proof}
\end{lemma}
...
\begin{align*}
a_i &= s_0 e^{-2 \alpha X_i} \\
r &= \frac{1}{s_0} e^{-\phi \alpha / \beta}
\end{align*}
\begin{proof}
...
\[
\log a_i = \log s_0 - 2 \alpha X_i
\]

which we can solve for $2 X_i$ to get

\[
2 X_i = \frac{1}{\alpha}(\log s_0 - \log a_i)
\]
...
&= a_i \sin \left( \frac{\beta}{\alpha}(\log s_0 - \log a_i) + \phi \right) + \mathcal{O}(e^{-2(\alpha + \gamma) X_i}) \\
&= a_i \sin \left( -\frac{\beta}{\alpha}(\log a_i - \log s_0 - \log e^{\phi \alpha / \beta} ) \right) + \mathcal{O}(e^{-2(\alpha + \gamma) X_i}) \\
&= a_i \sin \left( -\frac{\beta}{\alpha} \left(\log a_i + \log \left(\frac{1}{s_0} e^{-\phi \alpha / \beta} \right) \right) \right) + \mathcal{O}(e^{-2(\alpha + \gamma) X_i}) \\
&= a_i \sin \left( -\frac{\beta}{\alpha} \log a_i r \right) + \mathcal{O}(e^{-2(\alpha + \gamma) X_i}) \\
\end{align*}
...
&= a_{i-1} \sin \left( -\frac{\beta}{\alpha} \log a_{i-1} r \right) + \mathcal{O}(e^{-2(\alpha + \gamma) X_{i-1}}) \\
\end{align*}

Finally, we compute the order of the remainder in terms of the $a_i$ and the $r$.
