...
\[
A(\lambda) = \lim_{|x| \rightarrow \infty} A(q(x); 0)
\]
...
\begin{hypothesis}We have the following bounds on terms in our problem
\begin{align*}
...
SPECULATION: Since $G_i(\lambda(x)$ depends on $x$, we might be able to get a bound of the form $C e^{-\alpha X_1}e^{-\alpha |x|}$, as long as we stay on the appropriate ``piece'' of the problem. We will return to this later.
...
Next we note that the $c^\pm$ now have coefficients $e^{\pm \nu(\lambda) }T$. One of these is potentially bad, but we do not know which one. To get around this, we will add and subtract the other version. \\
