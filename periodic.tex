...
In this section we will look at multipulse solutions to KdV5 in the case where we have periodic boundary conditions. Note that we cannot use an exponential weight in this case since our domain is of finite length. For now, we will hypothesize that such solutions exist. There should be restrictions on $X_1$ and $X_2$ like in the multipulse case, and almost certainly we need to have $c > 1/4$.
...
What we will do here is construct the periodic double pulse based on San97. For now, we are assuming it exists. This will give us a useful expression for the periodic double pulse, which will then give us useful bounds for the stability problem. Since this method will not give us any restrictions on the $X_i$ (and we know there are some), we are assuming existence for now.\\
...
which we can write as a 4th order system.\\
...
\begin{equation}
|a_i^\pm| \leq C |Q^\pm(0, \beta_i^\pm)(\pm X_i)| \leq C e^{-\alpha X_i}
\end{equation}

which hold for $a_i^\pm$ as well as derivatives with respect to $\beta_i^\pm$.

\begin{proof}

First, plug in the fixed point equations and evaluate them at $\pm X_i$ to get
\begin{align*}
U_i^+(X_i) &= Q^+(0, \beta_i^+)(X_i) + P^u_+(X_i; \beta_i^+) a_i^+ + \int_0^{X_i} \Phi_+^s(x, y; \beta_i^+) G_i^+(y, V_i^+(y),\beta_i^+)dy \\ 
U_{i-1}^-(-X_i) &= Q^-(0, \beta_i^-)(-X_i) + P^s_-(-X_i; \beta_i^-) a_i^- + \int_0^{-X_i} \Phi_-^u(x, y; \beta_i^-) G_i^-(y, V_i^-(y),\beta_i^-)dy \\
\end{align*}

Adding and subtracting $P_0^{s/u}$ and recalling where the $a_i^\pm$ live, this becomes

\begin{align*}
U_i^+(X_i) &= Q^+(0, \beta_i^+)(X_i) + a_i^+ + (P^u_+(X_i; \beta_i^+) -  P^s_0)a_i^+ + \int_0^{X_i} \Phi_+^s(x, y; \beta_i^+) G_i^+(y, V_i^+(y),\beta_i^+)dy \\ 
U_{i-1}^-(-X_i) &= Q^-(0, \beta_i^-)(-X_i) + a_i^- + (P^s_-(-X_i; \beta_i^-) - P^u_0) a_i^- + \int_0^{-X_i} \Phi_-^u(x, y; \beta_i^-) G_i^-(y, V_i^-(y),\beta_i^-)dy \\
\end{align*}
...
&= a_i^+ - a_i^- + (P^u_+(X_i; \beta_i^+) -  P^s_0)a_i^+ - (P^s_-(-X_i; \beta_i^-) - P^u_0) a_i^-  \\
&+ Q^+(0, \beta_i^+)(X_i) - Q^-(0, \beta_i^-)(-X_i)\\
&+ \int_0^{X_i} \Phi_+^s(x, y; \beta_i^+) G_i^+(y, V_i^+(y),\beta_i^+)dy
- \int_0^{-X_i} \Phi_-^u(x, y; \beta_i^-) G_i^-(y, V_i^-(y),\beta_i^-)dy 
\end{align*}
...
&= a_i^+ - a_i^- + (P^u_+(X_i; \beta_i^+) -  P^s_0)a_i^+ - (P^s_-(-X_i; \beta_i^-) - P^u_0) a_i^-  \\
&+ Q^+(0, \beta_i^+)(X_i) - Q^-(0, \beta_i^-)(-X_i)\\
&+ \int_0^{X_i} \Phi_+^s(x, y; \beta_i^+) G_i^+(y, V_i^+(a_i^+, \beta_i^+)(y),\beta_i^+)dy \\
&- \int_0^{-X_i} \Phi_-^u(x, y; \beta_i^-) G_i^-(y, V_i^-(a_i^-, \beta_i^-)(y),\beta_i^-)dy 
\end{align*}
...
SOMEHOW WE SHOULD GET THE FOLLOWING ESTIMATE.
...
Before we do that, we will need to make a smooth change of coordinates near $Q(0)$. We will do that using the ``flow-box'' method. The idea is that we will straighten out the stable and unstable manifolds locally so that they lie along $Y^+$ and $Y^-$, and that all trajectories are parallel and have constant velocity.

\begin{lemma}

\begin{proof}
We know that $F(Q(0)) \neq 0$, i.e. the center of the pulse is not an equilibrium of the vector field $F$. We want to construct a three-dimensional section which is perpendicular to $F(Q(0))$ at $Q(0)$. First, recall that

\[
\R^4 = Z \oplus \R Q'(0) \oplus Y^+ \oplus Y^-
\]

First, we make a linear change of coordinates so that $Y^+$ and $Y^-$ are perpendicular to $Q'(0)$. Since $Z = \Psi(0)$ is already perpendicular to $Q'(0)$, we have $Y = Y^- \oplus Y^+ \oplus Z \perp Q'(0)$. Paramaterize $Y$ by the coordinates $(\beta^-, \beta^+, \gamma)$.\\

Since $F(Q(0)) \neq 0$, by continuity of $F$ we can find a ball $B_0$ around $Q(0)$ such that $F(x) \neq 0$ in $B_0 \cap Y$. Note that all trajectories $B_0$ cross $B_0 \cap Y$ in the same direction.\\

Next we construct a ``flow box'' near $Q(0)$. Let $S_y(P)$ be the solution operator for the original (nonlinear) ODE $U' = F(U)$, i.e. the operator which takes the initial condition $p$ and maps it to the point $H(y)$, where $H$ is the unique solution to $U' = F(U)$ such that $H(0) = P$.\\

Let $B \subset B
Define the map $\Psi$ from a neighborhood $N$ of the origin in $\R \times \R^3$ to a neighborhood of $Q(0)$ by

\begin{equation}
\Psi(y; \beta^-, \beta^-, \gamma) = S_y(Q(0) + Q^-(\beta^-, 0) + Q^-(\beta^-, 0) + \gamma \Psi(0))
\end{equation}

If we fix $(\beta^-, \beta^+, \gamma)$, then $\Psi(y; \beta^-, \beta^+, \gamma)$ maps to a trajectory of our system with IC $Q(0) + Q^-(\beta^-,0) + Q^-(\beta^+,0) + \gamma \Psi(0)$.\\ 

Near $Q(0)$, we have for small $x, \beta^\pm$ the stable and unstable manifolds are paramaterized by
