...
Every term we have gotten so far is higher order than $|\lambda|^2$, except for the higher-order Melnikov term, which is order $|\lambda|^2$. This is good.\\

The last term (the one involving $H$) is the following

\begin{align*}
\int_{-T}^0 &e^{-\nu(\lambda)y} v_-(0) \langle \lambda^2 H(y), w_-(y) \rangle dy - \int_T^0 e^{-\nu(\lambda)y} v_+(0) \langle \lambda^2 H(y), w_+(y) \rangle dy \\
&= \lambda^2 \left( \int_{-T}^0 e^{-\nu(\lambda)y} v_-(0) \langle H(y), w_-(y) \rangle dy + \int_0^T e^{-\nu(\lambda)y} v_+(0) \langle H(y), w_+(y) \rangle dy \right)
\end{align*}

Whatever this thing is, it is not a lower order term. A bound on this would be

\[
C |\lambda^2| e^{\nu(\lambda)T} \int_{-T}^T \langle H(y), w_\pm(y) \rangle dy 
\]

So this is actually a lower order term than the Melnikov term.
