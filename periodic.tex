...
\begin{enumerate}
	...
	\item The spectrum of $D_U F(0)$ contains simple eigehvalues $\pm \alpha \pm i \beta$, $\alpha, \beta > 0$. For any other eigenvalue $\nu$ of $D_U F(0)$ we have $|\text{Re }\nu| > \alpha$. Thus the real part of the spectrum of $D_U F(0)$ lies outside of $(-\alpha, \alpha)$.
...
In the case where the system is Hamiltonian (like we have here), we actually know the specific form of $\Psi(x)$.
...
\begin{proof}
BASED ON ASSUMPTIONS, this follows from Lemma 6.1 in San98. In our case, we do not have a parameter $\mu$, so the $\mu$-dependent terms are constant. If it turns out that $\gamma > 1$, we just take $\gamma = 1$.
\end{proof}
\end{lemma}
...
\begin{proof}
The Hamiltonian case follows from proof of Lemma 3.8 in SanStrut. Reversibility follows from Lemma 5.3 in San98.
\end{proof}
...
Taking the log of $a_i$, we have

\[
\log a_i = -2 \alpha (X_i - X)
\]

which we can solve for $2 \alpha X_i$ to get
...
substituting this into our expression for $\langle \Psi(X), Q(-X) \rangle$ and using the symmetry lemma, we have
...
For the remainder term $R_i$ from the above lemma, we have
...
In the next lemma, we solve for the $a_i$.
...
\begin{proof}
In the above lemma, take $r = r_n$, where
\[
r_n =  e^{-(2 \pi \alpha / \beta) n}
\]
and $n \in \N$. Noting that $(\beta / \alpha) \log r_n = 0$, the equations to solve become

\begin{align*}
a_{i-1} \sin \left( - \frac{\beta}{\alpha} \log a_{i-1} \right) - a_i \sin \left( - \frac{\beta}{\alpha} \log a_i \right) = \tilde{R}_i(a, r_n)
\end{align*}

Since the only dependence on $r_n$ is on the RHS, and we know that $\tilde{R}_i(a, r_n) = \mathcal{O}(r^{\tilde{\gamma}})$ from the prior lemma, we can send $r \rightarrow 0$ to get the system of equations

\begin{align*}
a_{i-1} \sin \left( - \frac{\beta}{\alpha} \log a_{i-1} \right) - a_i \sin \left( - \frac{\beta}{\alpha} \log a_i \right) = 0
\end{align*}

Define the function $G: R^3 \rightarrow R^2$ by

\begin{equation}
G_i(a_1, a_2, r) = a_{i-1} \sin \left( - \frac{\beta}{\alpha} \log a_{i-1} r \right) - a_i \sin \left( - \frac{\beta}{\alpha} \log a_i r \right) - \tilde{R}_i(a, r)
\end{equation}

If we take

\begin{equation}
a_i^0 = e^{-\frac{\alpha}{\beta} k_i^0 }
\end{equation}

for $k_i^0 \in \N_0$, from the above limit and from continuity, we have $G(a_1^0, a_2^0, 0) = 0$. Next, we take the Jacobian of $G$ with respect to the $a_i$ at $(a_1^0, a_2^0, 0)$. Note that when we do this, the remainder term $\tilde{R}_i(a, r)$ does not contribute since its derivative with respect to $a_i$ is order $r^{\tilde{\gamma}}$, which is 0 at $r = 0$.\\

Since

\begin{align*}
\frac{\partial}{\partial a_i} a_i \sin \left( - \frac{\beta}{\alpha} \log a_i r \right)\Big|_{a_i = a_i^0, r = 0}
&= \sin \left( - \frac{\beta}{\alpha} \log a_i r \right)\Big|_{a_i = a_i^0, r = 0} - \frac{\beta}{\alpha}\cos \left( - \frac{\beta}{\alpha} \log a_i r \right)\Big|_{a_i = a_i^0, r = 0}\\
&=(-1)^{k_i^0 + 1}\frac{\beta}{\alpha}
\end{align*}

where we took the limit as $r \rightarrow 0$ as we did above, the Jacobian of $G$ with respect to $(a_1, a_2)$ at $(a_1^0, a_2^0, 0)$ is

\begin{equation}
DG_{(a_1, a_2)}G(a_1^0, a_2^0, 0) =
\begin{pmatrix}
