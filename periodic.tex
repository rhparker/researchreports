...
First, note that $|\nu(\lambda)|$ is small compared to $\alpha$, so the $\alpha - \nu(\lambda)$ term in the denominator cannot blow up. Next, recall that $x \in [-X_{i-1}, 0]$ on this piece, so $X_{i-1} + x \geq 0$ on this piece. If $\nu(\lambda) < 0$, then $e^{\nu(\lambda)(X_{i-1} + x)} \leq 1$. If $\nu(\lambda) \geq 0$, then $e^{\nu(\lambda)(X_{i-1} + x)} \leq e^{\nu(\lambda)(X_{i-1}}$. Thus, we have the following bound, which is uniform in $x$ and is independent of the sign of $\nu(\lambda)$.
...
\begin{align*}
||W_1(\lambda)(a,b,c,d)_i^-|| &\leq C ( |a| + |b| + e^{\nu(\lambda)X_{i-1}}|c_{i-1}| + |\lambda|^2 |d| ) \\
...
\begin{align*}
|W_i^-(x)| &\leq |(L_1(\lambda)W)_i^-(x)| + |L_2(\lambda)(a,b,c,d)_i^-(x)| \\
&\leq C \Big( ||G|| ||W_i^-|| + e^{-\alpha(x + X_{i-1})}|a_{i-1}^-| + |b| + e^{\nu(\lambda)x} |\tilde{c}_{i-1}^-| + |\lambda|^2 |d| ) \\
&\leq \Big( ||G|| (|a| + |b| + e^{\nu(\lambda)X_{i-1}}|c_{i-1}| + |\lambda|^2 |d|) + e^{-\alpha(x + X_{i-1})}|a_{i-1}^-| + |b| + e^{\nu(\lambda)x} |\tilde{c}_{i-1}^-| + |\lambda|^2 |d| )
\end{align*}

Thus we have

\begin{align*}
|W_i^-(x)| &\leq \Big( ||G|| |a| + e^{-\alpha(x + X_{i-1})}|a_{i-1}^-| + |b| + ||G|| e^{\nu(\lambda)X_{i-1}}|c_{i-1}| + e^{\nu(\lambda)x} |\tilde{c}_{i-1}^-| + |\lambda|^2 |d| ) \\
\end{align*}

Similarly, 

\begin{align*}
|W_i^+(x)| &\leq \Big( ||G|| |a| + e^{-\alpha(x - X_i)}|a_i^+| + |b| + ||G|| e^{\nu(\lambda)X_i}|c_i| + e^{\nu(\lambda)x} |\tilde{c}_i^+| + |\lambda|^2 |d| ) \\
\end{align*}
...
&\leq C ||G|| \left( |a| + |b| + |\lambda|^2 |d| + \int_0^{X_i} e^{-\alpha(X_i - y)}e^{-\alpha y} e^{\nu(\lambda)X_i}|c_i^+| dy \right)
\end{align*}

For the integral on the RHS, we have

\begin{align*}
\int_0^{X_i} e^{-\alpha(X_i - y)}e^{-\alpha y} e^{\nu(\lambda)X_i}|c_i^+| dy
&\leq |c_i^+| \int_0^{X_i} e^{-(\alpha - \nu(\lambda))X_i}e^{\alpha y} e^{-\alpha y} dy \\
&\leq |c_i^+| \int_0^{X_i} e^{-(\alpha - \nu(\lambda))X_i}e^{\tilde{\alpha} y} e^{-\tilde{\alpha} y} dy \\
&\leq |c_i^+| \int_0^{X_i} e^{-(\alpha - \nu(\lambda))(X_i - y)} e^{-\tilde{\alpha} y} dy \\
&\leq |c_i^+| \int_0^{X_i} e^{-(\alpha - \nu(\lambda))(X_i - y)} dy \\
&\leq |c_i^+|
\end{align*}

where we used the fact that $|\nu(\lambda)| \leq \alpha - \tilde{\alpha}$. Note that this works regardless of the sign of the real part of $\nu(\lambda)$. 

Thus we have

\begin{align*}
&\left| \int_0^{X_i} \Phi^s_+(X_i, y; \lambda) G_i^+(y) W_i^+(y) \right| \\
&\leq C ||G|| \left( |a| + |b| + |c_i^+| + \lambda|^2 |d| \right)
\end{align*}
...
NOTE: THIS LEMMA IS NOT TRUE AS WRITTEN SINCE IT RELIES ON A BOUND WHICH WE DO NOT IN FACT HAVE.
...
EVERYTHING IS OK TO HERE. THIS NEXT PART IS NOT TRUE SINCE IT RELIES ON A BOUND WE DO NOT HAVE.
...
&\leq C e^{-\alpha X_i} ( (e^{-\alpha X_i} + ||G||)|b| + ( p_4(X_i; \lambda) + ||G|| )|c_i| \\
...
|c_i| &\leq C (|c_i^+| + |c_i^-|) \\ 
&\leq C(e^{\nu(\lambda) X_i}|e^{-\nu(\lambda) X_i} c_i^+| + e^{-\nu(\lambda) X_i}|e^{\nu(\lambda) X_i} c_i^-|)\\
&= C(e^{\nu(\lambda) X_i}|\tilde{c}_i^+| + e^{-\nu(\lambda) X_i}|\tilde{c}_i^-|) 
\end{align*}

Since we will be multiplying this by $e^{-\alpha X_i}$, this bound becomes

\begin{align*}
e^{-\alpha X_i}|c_i| \leq e^{-(\alpha - \nu(\lambda)) X_i} |\tilde{c}_i|
\end{align*}

Substituting this above and using the $B_1$ bound, we have

\begin{align*}
\langle \Psi(0), &\Phi^u_+(0, X_i; \lambda) A_2(\lambda)_i^+(b,c,d) \rangle \\
&\leq C e^{-\alpha X_i} (e^{-\alpha X_i} + ||G||)|b| 
+ e^{-(\alpha - \nu(\lambda)) X_i}( p_4(X_i; \lambda) + ||G|| )|\tilde{c}_i| \\
&+ e^{-\alpha X_i}(e^{-\tilde{\alpha} X_i} + ||G||) |\lambda^2| |d| 
+ e^{-\alpha X_i}(p_1(X_i; \lambda) + ||G|| )|D_i||d|) ) \\
&\leq C e^{-\alpha X_i} (e^{-\alpha X_i} + ||G||)|B_1(\lambda)(d)|
+ e^{-(\alpha - \nu(\lambda)) X_i}( p_4(X_i; \lambda) + ||G|| )|B_1(\lambda)(d)| \\
&+ e^{-\alpha X_i}(e^{-\tilde{\alpha} X_i} + ||G||) |\lambda^2| |d| 
+ e^{-\alpha X_i}(p_1(X_i; \lambda) + ||G|| )|D_i||d|) ) \\
&\leq C e^{-\alpha X_i} (e^{-\alpha X_i} + ||G||)( (e^{-\alpha X_i} + ||G||) |D_i|+ (e^{-\alpha X_{i-1}} + ||G||) |D_{i-1}| +|\lambda|^2 )|d| \\
&+ e^{-(\alpha - \nu(\lambda)) X_i}( p_4(X_i; \lambda) + ||G|| )( (e^{-\alpha X_i} + ||G||) |D_i|+ (e^{-\alpha X_{i-1}} + ||G||) |D_{i-1}| +|\lambda|^2 )|d| \\
&+ e^{-\alpha X_i}(e^{-\tilde{\alpha} X_i} + ||G||) |\lambda^2| |d| 
+ e^{-\alpha X_i}(p_1(X_i; \lambda) + ||G|| )|D_i||d|) ) 
\end{align*}
...
+ e^{-(\alpha - \nu(\lambda)) X_i}( e^{-\alpha X_1} + ||G|| + p_1(X_i; \lambda))|D| )|d|
\Big) 
\end{align*}

where recall that we are taking $X_2 \geq X_1$. Thus, we conclude

\begin{align*}
\langle \Psi(0), &\Phi^u_+(0, X_i; \lambda) a_i^+ \rangle \\
&= \langle \Psi(X_i), P^u_0 D_i d \rangle + \mathcal{O}\Big(e^{-(\alpha - \nu(\lambda)) X_i}(|\lambda|^2 
+ ( e^{-\alpha X_1} + ||G|| + p_1(X_i; \lambda) + |\lambda|)|D| )|d|\Big)
\end{align*}
...
+ \mathcal{O}\Big(e^{-(\alpha - \nu(\lambda)) X_{i-1}}(|\lambda|^2 
+ ( e^{-\alpha X_1} + ||G|| + p_1(X_{i-1}; \lambda) + |\lambda|)|D| )|d|\Big)
\end{align*}
...
&\leq C p_5(\lambda)( (e^{-\alpha X_i} + ||G||) |D_i|+ (e^{-\alpha X_{i-1}} + ||G||) |D_{i-1}| +|\lambda|^2 )|d|\\
&\leq C p_5(\lambda) ( (e^{-\alpha X_1} + ||G||)|D| +  |\lambda|^2 )|d|
\end{align*}

where we have taken $X_2 \geq X_1$. 
...
&\leq C |\lambda| ( (e^{-\alpha X_i} + ||G||) |D_i|+ (e^{-\alpha X_{i-1}} + ||G||) |D_{i-1}| +|\lambda|^2 )|d| \\
&\leq C |\lambda| ( (e^{-\alpha X_1} + ||G||) |D| + |\lambda|^2 )|d| 
\end{align*}
...
Next, we look at the integral terms in the jump $\xi_i$. First, we look at the ``noncenter'' integral terms. Before we do that, we will need an improved estimate for $W$.

\begin{lemma}
We have the following improved, piecewise, $x$-dependent estimate for $W$
\begin{proof}
...
Integrals not involving $H$. For these we only do the ``minus'' half. The ``plus'' half is similar
