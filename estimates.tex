...
\begin{align*}
| e^{-\eta x} W_i^-(x)| &\leq C\left(\left( e^{\eta X_{i-1}} e^{-\alpha(x + X_{i-1})} + e^{-\alpha L} + e^{\eta L}(|G| + |\lambda|) \right)|D_1| + |\lambda|^2 \right) |d| \\
| e^{\eta x} W_i^+(x)| &\leq C\left(\left( e^{\eta X_{i}} e^{\alpha(x - X_{i})} + e^{-\alpha L} + e^{\eta L}(|G| + |\lambda|) \right)|D_1| + |\lambda|^2 \right) |d| \\
\end{align*}
...
\begin{align*}
\left| \int_0^{X_i} \langle \Psi(x), G_i^+(x) W_i^+(x) \rangle dx \right| &\leq C|G| \left( \left( e^{-\alpha L} + e^{\eta L}(|G| + |\lambda|)  \right) |D_1| + |\lambda|^2 \right)|d|\\
...
Using our estimate for $|e^{\pm \eta x} W_i^\pm(x)|$, and taking $X_i = L$ since we have a double pulse, we have
...
&\leq C \int_0^{X_i} e^{-\alpha x} \int_x^{X_i} e^{-\eta y} \left(\left( e^{\eta L} e^{\alpha(y - L)} + e^{-\alpha L} + e^{\eta L}(|G| + |\lambda|) \right)|D_1| + |\lambda|^2 \right) |d|  dy dx \\
&= C \int_0^{X_i} e^{-\alpha x} \int_x^{X_i} e^{(\alpha - \eta)(y - L)} |D_1| |d| dy dx \\
&+ C \int_0^{X_i} e^{-\alpha x} \int_x^{X_i} e^{-\eta y} \left(\left( e^{-\alpha L} + e^{\eta L}(|G| + |\lambda|) \right)|D_1| + |\lambda|^2 \right) |d| dy dx
\end{align*}

Let's do the first integral first. Since we are dealing with a double pulse, we can take $X_i = L$.

\begin{align*}
\int_0^{L} e^{-\alpha x} \int_x^{L} e^{(\alpha - \eta)(y - L)} dy dx &= \frac{1}{\alpha - \eta} \int_0^{L} e^{-\alpha x} \left(1 - e^{(\alpha - \eta)(x - L)}\right) dx \\
&\leq \frac{1}{\alpha - \eta} \int_0^{\infty} e^{-\alpha x} dx \\
&= \frac{1}{\alpha(\alpha - \eta)}
\end{align*}

For the second integral,

\begin{align*}
\int_0^{X_i} &e^{-\alpha x} \int_x^{X_i} e^{-\eta y} \left(\left( e^{-\alpha L} + e^{\eta L}(|G| + |\lambda|) \right)|D_1| + |\lambda|^2 \right) |d| dy dx \\
&\leq \left(\left( e^{-\alpha L} + e^{\eta L}(|G| + |\lambda|) \right)|D_1| + |\lambda|^2 \right) |d|  \int_0^{X_i} e^{-\alpha x} \int_x^{\infty} e^{-\eta y} dy dx \\
&\leq \frac{1}{\eta}\left(\left( e^{-\alpha L} + e^{\eta L}(|G| + |\lambda|) \right)|D_1| + |\lambda|^2 \right) |d|  \int_0^{\infty} e^{-(\alpha + \eta) x} dx \\
&= \frac{1}{\eta(\alpha + \eta)}\left(\left( e^{-\alpha L} + e^{\eta L}(|G| + |\lambda|) \right)|D_1| + |\lambda|^2 \right) |d| 
\end{align*}

Combining these, we have

\begin{align*}
\left| \int_0^{X_i} \langle \Psi(x), (K_i^+ W_i^+)(x)\rangle dx \right| &\leq C |D_1| |d| + C\left(\left( e^{-\alpha L} + e^{\eta L}(|G| + |\lambda|) \right)|D_1| + |\lambda|^2 \right) |d|  \\
&= C\left( 1 + e^{\eta L}(|G| + |\lambda|) |D_1| + |\lambda|^2 \right) |d| \\
\end{align*}
