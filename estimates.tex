...
\begin{align*}
\left| \int_0^{X_i} \langle \Psi(x), G_i^+(x) W_i^+(x) \rangle dx \right| &\leq C|G| \int_0^{\i} \left(  e^{-(\tilde{\alpha} - \alpha)x} e^{-\alpha X_i} +  e^{-\tilde{\alpha} x} \left( \left(e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) \right) |d|dx \\
