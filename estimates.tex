...
\begin{align*}
| W_i^-(x)| &\leq C\left(\left( e^{-\alpha(x + X_{i-1})}e^{-\eta L} + e^{-(\alpha + \eta) L} + |G| + |\lambda| ) \right) |D_1^{\eta}| + |\lambda|^2 \right) |d| \\
| W_i^+(x)| &\leq C\left(\left( e^{\alpha(x - X_{i})}e^{-\eta L} + e^{-(\alpha + \eta) L} + |G| + |\lambda| ) \right) |D_1^{\eta}| + |\lambda|^2 \right) |d|
\end{align*}
...
&\leq C\left(\left( e^{-\alpha(x + X_{i-1})}e^{-\eta L} + e^{-(\alpha + \eta) L} + |G| + |\lambda| ) \right) |D_1^{\eta}| + |\lambda|^2 \right) |d|
\end{align*}
...
We can do the same thing in the weighted norm.

\begin{lemma}
We have the following estimates for $|e^{\pm \eta x} W_i^\pm(x)|$.

\begin{align*}
| e^{-\eta x} W_i^-(x)| &\leq C\left(\left( e^{-\alpha(x + L)} + e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d| \\
| e^{\eta x} W_i^+(x)| &\leq C\left(\left( e^{\alpha(x - L)} + e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d| 
\end{align*}
where $\alpha = \min(\alpha^s, \alpha^u)$.
...
e can use the estimate we have for $L_1(\lambda)$ together with an improved estimate for $L_2(\lambda)$. For $L_1(\lambda)$, we have for the ``negative'' piece

\[
|e^{-\eta x} (L_1(\lambda) W_i^-)(x) | \leq C\left(|G| +|\lambda|\right)||W||_\eta
\]

For $L_2(\lambda)$ we will use the following estimate for the negative piece.

\begin{align*}
| e^{-\eta x} (L_2(\lambda)(a, b, d)_i^-)(x)| &\leq C \left( e^{\eta X_{i-1}} e^{-\alpha^s(x + X_{i-1})} |a^-_{i-1}| + |b_i^-| + |\lambda|^2 |d_i| \right)
\end{align*}

Combining these, we have

\begin{align*}
|e^{-\eta x} W_i^-(x)| &\leq C \left( e^{\eta X_{i-1}} e^{-\alpha^s(x + X_{i-1})} |a^-_{i-1}| + |b_i^-| + \left(|G| +|\lambda|\right)||W||_\eta + |\lambda|^2 |d_i| \right)
\end{align*}

Substituting in $A_3(\lambda)$, $B_1(\lambda)d$, and $W_3(\lambda)d$, this estimate becomes

\begin{align*}
| e^{-\eta x} W_i^-(x)| &\leq C ( e^{\eta X_{i-1}} e^{-\alpha^s(x + X_{i-1})} \left( e^{-\eta L}|\lambda^2| + |D_1| \right)|d| \\
&+ (|\lambda|^2 + (e^{-\alpha L} + |G| + |\lambda|)|D_1| )|d| \\
&+ \left(|G| +|\lambda|\right)\left( |\lambda|^2 + e^{\eta L}|D_1|\right)|d| + |\lambda|^2 |d| )\\
&\leq C\left(\left( e^{\eta X_{i-1}} e^{-\alpha(x + X_{i-1})} + e^{-\alpha L} + |G| + |\lambda| + e^{\eta L}(|G| + |\lambda|) \right)|D_1| + |\lambda|^2 \right) |d| \\
\end{align*}

Now, take $X_{i-1} = L$ and substitute in 
...
\begin{align*}
| e^{-\eta x} W_i^-(x)| &\leq C\left(\left( e^{\eta L} e^{-\alpha(x + L)} + e^{-\alpha L} + |G| + |\lambda| + e^{\eta L}(|G| + |\lambda|) \right)e^{-\eta L} |D_1^{\eta}| + |\lambda|^2 \right) |d| \\
&\leq C\left(\left( e^{-\alpha(x + L)} + e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d| 
\end{align*}
Similarly,
...
\begin{align*}
\left| \int_0^{X_i} \langle \Psi(x), G_i^+(x) W_i^+(x) \rangle dx \right| &\leq C|G| \left( \left( e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right)|d| \\
\left| \int_{-X_{i-1}}^0 \langle \Psi(x), G_i^-(x) W_i^-(x) \rangle dx \right| &\leq C|G| \left( \left( e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right)|d|
\end{align*}
...
&\leq C|G| \int_0^{X_i} e^{-\tilde{\alpha} x} \left(\left( e^{\alpha(x - X_{i})}e^{-\eta L} + e^{-(\alpha + \eta) L} + |G| + |\lambda| ) \right) |D_1^{\eta}| + |\lambda|^2 \right) |d|dx 
\end{align*}
...
&\leq C|G| \left(  \frac{1}{\tilde{\alpha} - \alpha}e^{-\alpha X_i}e^{-\eta L} + \frac{1}{\tilde{\alpha}}\left( \left(e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) \right)|d| \\
&\leq C|G| \left( \left( e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right)|d|
\end{align*}
...
This bound is fine and good, but we will actually need a better bound. We can do this using the sharper estimate for $|e^{\pm \eta x} |W_i^\pm(x)|$

\begin{lemma}
We have the following bound on these integrals involving the integration operator.

\begin{align*}
\left| \int_0^{X_i} \langle \Psi(x), (K_i^+ W_i^+)(x)\rangle dx \right| &\leq C\left(\left( e^{-\eta L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d| \\
\left| \int_{-X_{i-1}}^0 \langle \Psi(x), (K_i^- W_i^-)(x)\rangle dx \right| &\leq C\left(\left( e^{-\eta L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d|
\end{align*}
...
\left| \int_0^{X_i} \langle \Psi(x), (K_i^+ W_i^+)(x)\rangle dx \right| &\leq C \int_0^{X_i} e^{-\alpha x} \int_x^{X_i} w_i^+(y) dy dx \\
&= C \int_0^{X_i} e^{-\alpha x} \int_x^{X_i} e^{-\eta y} |e^{\eta y} w_i^+(y)| dy dx\\
&\leq C \int_0^{X_i} e^{-\alpha x} \int_x^{X_i} e^{-\eta y} \left(\left( e^{\alpha(y - L)} + e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d| dy dx \\
&= C \int_0^{X_i} e^{-\alpha x} \int_x^{X_i} e^{-\eta y} e^{\alpha(y - L)} |D_1^{\eta}| |d| dy dx \\
&+ C \int_0^{X_i} e^{-\alpha x} \int_x^{X_i} e^{-\eta y} \left(\left( e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d| dy dx
\end{align*}

The first integral is more complicated, so let's do that one first. Since we are dealing with a double pulse, we can take $X_i = L$.

\begin{align*}
\int_0^{X_i} e^{-\alpha x} \int_x^{X_i} e^{-\eta y} e^{\alpha(y - L)} dy dx &\leq e^{-\eta L} \int_0^{L} e^{-\alpha x} \int_x^{L} e^{(\alpha - \eta)(y - L)} dy dx\\
&= \frac{e^{-\eta L}}{\alpha - \eta} \int_0^{L} e^{-\alpha x} \left(1 - e^{(\alpha - \eta)(x - L)}\right) dx \\
&\leq \frac{e^{-\eta L}}{\alpha - \eta} \int_0^{\infty} e^{-\alpha x} dx \\
&= \frac{e^{-\eta L}}{\alpha(\alpha - \eta)}
\end{align*}

For the second integral,

\begin{align*}
\int_0^{X_i} &e^{-\alpha x} \int_x^{X_i} e^{-\eta y} \left(\left( e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d| dy dx \\
&\leq \left(\left( e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d| \int_0^{X_i} e^{-\alpha x} \int_x^{\infty} e^{-\eta y} dy dx \\
&\leq \frac{1}{\eta}\left(\left( e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d| \int_0^{\infty} e^{-(\alpha + \eta) x} dx \\
&= \frac{1}{\eta(\alpha + \eta)}\left(\left( e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d|
\end{align*}

Combining these, we have

\begin{align*}
\left| \int_0^{X_i} \langle \Psi(x), (K_i^+ W_i^+)(x)\rangle dx \right| &\leq C e^{-\eta L} |D_1^{\eta}| |d| + C\left(\left( e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d| \\
&= C\left(\left( e^{-\eta L} + e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d| \\
&= C\left(\left( e^{-\eta L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d|
\end{align*}
