...
\item Using $Y_0$ as the initial condition for the variational equation \eqref{vareq2} gives us the bounded solution $V_0(x)$. For the decay to $V_0$, we might need the gap lemma (which gives this on $\R^\pm$ separately) and the symmetry of the problem.
...
Each of the $2n - 2$ points $\sqrt{\mu_k/M}$ (for $k = 1, \dots, n-1$) is located at least a distance $\epsilon/X$ from all of the points $\lambda(X,k)$ (for $k \in \Z$), where $\epsilon < 1$ will be specified later.
\end{hypothesis}

We need one more hypothesis for part of the following theorem to hold. Essentially, it says that the length of the domain $2X$ cannot be exponentially large in comparison to $X_m$.

\begin{hypothesis}\label{Xrestrictionhyp}
For $X = X_0 + X_1 + \dots + X_{n-1}$, we have the following restriction.
\begin{equation}
X \leq C e^{\alpha X_m}
\end{equation}
...
We can now state the following theorem, in which we locate and count the eigenvalues of \eqref{PDEeig}.

\begin{theorem}
Let $\delta > 0$ be defined as in Theorem \ref{blockmatrixtheorem}. Assume the same hypotheses as in Theorem \ref{perexist}, and let $q_{np}(x)$ be a periodic $n-$pulse solution constructed with lengths $X_0, \dots, X_{n-1}$ according to Theorem \ref{perexist}, where $X_m = \min\{ X_0, \dots X_{n-1}\}$ and $e^{-\alpha X_m} < \delta$. Assume the same hypotheses as in Theorem \ref{blockmatrixtheorem}. Assume Hypothesis \ref{epsilonballs}, with $\epsilon$ to be chosen below. Then the following are true concerning the eigenvalues of \eqref{PDEeig}. 

\begin{enumerate}[(i)]

\item There is an eigenvalue at 0 with algebraic multiplicity 2. The kernel eigenfunction is $\partial_x q_{np}(x)$, and the generalized kernel eigenfunction is $t_{np}(x)$.

\item Choose $\epsilon = e^{-\alpha X_m/2}$ in Hypothesis \ref{epsilonballs}. Then, for sufficiently large $X_m$, there are $2n-2$ nonzero interaction eigenvalues which come in pairs and are either real or purely imaginary. These interaction eigenvalues are given by $\lambda = \pm \lambda^i(X_m; k)$, $k = 1, \dots, n-1$, where
...
\item For a radius $\tilde{\delta}$ which may be slightly smaller than $\delta$ and for sufficiently large $X_m$, there are exactly $2n + 2k + 1$ eigenvalues inside $B(0, \tilde{\delta}) \subset \C$ (counting multiplicity), where $n$ is the number of pulses in the multi-pulse, and $k$ is a positive integer depending on $X$. From Hamiltonian symmetry, there is an additional eigenvalue at 0. 

\item In Hypothesis \ref{epsilonballs}, choose $\epsilon = e^{-\alpha X_m/(n+1)}$, where $n$ is the number of pulses in the multi-pulse. (This is a larger $\epsilon$ than in part (i)). In addition, assume Hypothesis \ref{Xrestrictionhyp}. Then for any positive integer $k$ such that $k/X$ is sufficiently small, there is a pair of purely imaginary ``essential spectrum'' eigenvalues. These are given by $\lambda = \pm \lambda^c(X_m; X, k)$, where
...
\item Take the same assumptions as in part (iv). Then there are no eigenvalues inside $B(0, \tilde{\delta}) \subset \C$ other than those which have already been discussed, where $\tilde{\delta}$ is the same as in part (iii).
