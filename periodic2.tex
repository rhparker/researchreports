...
\begin{enumerate}
	\item The matrix $A - \lambda^2 M I$ is singular at $\lambda = 0$ (algebraic multiplicity 2) and at the $2n-2$ points $\lambda = \pm \sqrt{\mu_1/M}, \dots, \pm \sqrt{\mu_{n-1}/M}$, where $0, \mu_1, \dots, \mu_{n-1}$ are the eigenvalues of $A$.

	\item The matrix $K(\lambda)$ is singular at $\lambda = \pm \lambda^K(X,k)$, where
...
which is a complete metric space. Then, to construct a periodic $n-$pulse,

\begin{enumerate}
	\item Choose an integer $n \geq 2$ (the number of pulses) 
	\item Choose a sequence of nonnegative integers $m_0, \dots, m_{n-1}$ with the restriction that at least one of them must be 0. Use the $m_j$ to define the length parameters 
	\[ 
	...
	\item Then there exists $r_0 > 0$ such that 
		\begin{itemize}
		\item For every scaling parameter $r \in \mathcal{R}$ with $r < r_0$, there is a unique periodic $n-$pulse solution $q_{np}(x)$ which is given by the $n$ length parameters $b_j(r; m_j, \theta)$, where
		\begin{align*}
...
With this, we can state the following theorem. Here is a rough version.

\begin{theorem}
Let $\delta > 0$ be defined as in Theorem \ref{blockmatrixtheorem} and let $q_{np}(x)$ be a periodic $n-$pulse solution constructed as described above.\\

Then the following are true.

\begin{enumerate}[(i)]

\item $\partial_x q_{np}(x)$ is an eigenfunction with eigenvalue 0, and $t_{np}(x)$ is its generalized eigenfunction with eigenvalue 0. We also have another eigenfunction with eigenvalue 0 (MORE DETAILS LATER).

\item Assume
	\begin{itemize}
		\item The eigenvalues $(0, \mu_1, \dots, \mu_{n-1})$ of $A$ are distinct.
		\item The singular points of $K(\lambda)$ and the nonzero singular points of $A - \lambda^2 M I$ are separated by at least $\epsilon$, where
		\[
		\epsilon = \frac{1}{n |\log r| + C_b} r^{1/4}
		\]
		where $C_b$ is a constant which depends only on the $b_j^*(m_j, \theta) $, which are fixed. Note that the singular points of $K(\lambda)$ are approximately regularly spaced on the imaginary axis at intervals $\frac{\pi}{n |\log r| + C_b}$.
	\end{itemize}
	Then there exists $r_1 \leq r_0$ such that for $r \leq r_1$, there are $n - 1$ pairs of interaction eigenvalues given by $\lambda = \pm \lambda^{\text{int}}_j(r)$, $j = 1, \dots, n-1$, where

	\begin{align*}
	\lambda^{\text{int}}_j(r) = \sqrt{\mu_j / M} + \mathcal{O}(r^{1 + \gamma/2 \alpha})
	\end{align*}

	These interaction eigenvalues pairs are either real or purely imaginary.\\

	In addition, there exists a natural number $m_0$ with the following property. If for some $j$, $m_j - m_k \geq m_0$ for all $k \neq j$, then there exists $\tilde{r}_1 \leq r_1$ such that for all $r < \tilde{r}_1$, 

	\begin{itemize}
	\item For $M_2 > 0$, there are $n_{\text{even}}$ purely imaginary pairs of interaction eigenvalues.
	\item For $M_2 > 0$, there are $n_{\text{odd}}$ real pairs of interaction eigenvalues.
	\item It is the other way around for $M_2 < 0$
	\end{itemize}

	where $n_{\text{even}}$ is the number of even $m_k \neq m_j$ and $n_{\text{odd}}$ is the number of odd $m_k \neq m_j$. $M_2$ is the higher order Melkinov integral.
...
\item Assume
	\begin{itemize}
		\item The singular points of $K(\lambda)$ and the nonzero singular points of $A - \lambda^2 M I$ are separated by at least $\epsilon$, where
		\[
		\epsilon = \frac{1}{n |\log r| + C_b} r^{(3n-2)/6(n+3)}
		\]
		where $C_b$ the same constant as in the previous section. Note that for $r \geq 2$, $(3n-2)/6(n+3) > 0$.
	\end{itemize}

	Then there exists $r_2 \leq r_1$ such that for sufficiently small $k/X$, $k \neq 0$, there exist pairs of purely imaginary ``essential spectrum'' eigenvalues, which are given by $\lambda = \pm \lambda^{ess}(X, k; r)$, where

	\begin{equation}\label{essspeceigs}
	\lambda^{ess}(X, k; r_m) = -c_0 \frac{k \pi i}{X} + \mathcal{O}\left(r\right)
	\end{equation}

	The remainder term cannot move these off of the imaginary axis.

\item For a radius $\tilde{\delta}$ which may be slightly smaller than $\delta$, and with the same assumptions as above, there are exactly $2n + 2k + 1$ eigenvalues inside $B(0, \tilde{\delta}) \subset \C$ (counting multiplicity), where $n$ is the number of pulses in the multi-pulse, and $k$ is a positive integer depending on $X$. Thus there are no eigenvalues inside $B(0, \tilde{\delta}) \subset \C$ other than those which have already been accounted for.
