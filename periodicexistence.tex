...
\begin{enumerate}
	\item If $(c^*(0, r_m)$ is on the left of $b^*$, then let $\Gamma$ be the closed curve in the zero set of $G(a_0, a_1, 0) = F(a_0, a_1)$ connecting $(1, 0)$ to $b^*$. In other words, $\Gamma$ is entire the lower arm of the pitchfork for $b^*$. $\Gamma$ can be written as a graph over $a_1$, i.e. we have $\Gamma = (\gamma(a_1), a_1)$ for $a_1 \in [0, b^*]$. We also have $G_{a_0}(\gamma(a_1), a_1, 0) = F_{a_0}(\gamma(a_1), a_1) \neq 0$ in the interior of $\Gamma$  

	\item If $(c^*(0, r_m)$ is on the right of $b^*$, then let $\Gamma$ be the two-piece curve in the zero set of $G(a_0, a_1, 0) = F(a_0, a_1)$ connecting $(1, 0)$ to $b^*$ and then proceeding to $(c^*(0, r_m)$ along the diagonal. 
