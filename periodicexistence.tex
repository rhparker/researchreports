...
It is easy to see from \eqref{Fxy} that $F(-x, y) = -F(x, y)$, thus we have the required symmetry for a pitchform bifurcation; $y$ will be the bifurcation parameter. In our new coordinates, the bifurcations should occur at $(x_0, y_0) = \left(0, p^*_k \right)$.\\
...
We can combine the previous two lemmas into the following theorem.
...
We can also obtain a uniform result. The idea is that for sufficiently small $b_{n-1}^0$ (i.e. sufficiently large $m_{n-1}$) we can uniformly bound the $\partial_{b_j} G_j$ away from 0. To do this, we use the parameterization and bounds from Lemma \ref{thetaparam}. For $j = 0, \dots, n-2$, choose $b_j^0$ (equivalently, choose $m_j$). Then we have 
...
which is independent of $\theta$. Choose $m_{m-1}$ sufficiently large so that for $j = 0, \dots, n-2$,
\[
...
Thus, there exists $M \in \N$ such that for $m_{n-1} \geq M$, we have the bound
...
\begin{equation}
\mathcal{B} = \left\{ \exp\left( -\frac{m \pi}{\rho} \right) : m \geq M\right\} \cup {0}
\end{equation}

which is a complete metric space. Let $I = [-\arctan \rho, \pi - \arctan \rho]$. Redefine the map $G$ as $G: \mathcal{R}^{n-2} \times \mathcal{B} \times I \rightarrow \R^{n-1}$ with $G = (G_0, \dots, G_{n-2})^T$, where 
\begin{align*}
G_j(b_0, &\dots, b_{n-2}, b_{n-1}, \theta, r) = b_j e^{-\frac{1}{\rho} \theta_j^*(\theta)} \sin \left( -\rho \log \left( b_j e^{-\frac{1}{\rho} \theta_j^*(\theta)} \right) \right) \\
&- b_{n-1} e^{-\frac{1}{\rho} \theta} \sin \left( -\rho \log \left( b_{n-1} e^{-\frac{1}{\rho} \theta} \right) \right) + \mathcal{O}(r^{\gamma / 2 \alpha})
\end{align*}

All we did was make the parameterization from Lemma \ref{thetaparam} explicit. For all $b_{n-1} \in \mathcal{B}$ and $\theta \in I$, $G(b_0^0, \dots, b_{n-2}^0, b_{n-1}, \theta, 0) = 0$ and $||D_{b_0, \dots, b_{n-2}} G(b_0^0, \dots, b_{n-2}^0, b_{n-1}, \theta, 0)||$ is diagonal with all eigenvalues having magnitude greater than $\rho/2$, thus uniformly bounded away from 0. Thus we can use the uniform contraction mapping principle to find $\tilde{r}_0 > 0$ with the following property. For all $r \in \mathcal{R}$ with $r < \tilde{r}_0$, $b_{n-1} \in \mathcal{B}$, and $\theta \in I$, there are smooth functions $b_j(r; m_j, \theta)$ with $b_j(0; m_j, \theta) = b_j^*(m_j, \theta)$ such that
\[
G(b_1(r; m_1, \theta), \dots, b_{n-2}(r; m_{n-2}, \theta) ) = 0
\]

These solution are the same ones as found in the first section, and the bounds are exactly the same.
\end{proof}
\end{theorem}
