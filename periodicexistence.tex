...
\begin{proof}
Since we are in a periodic domain, we can WLOG take $b_{n-1}^0 \leq b_j^0$ (equivalently, $m_{n-1} \geq m_j$) for $j = 0, \dots, n-2$. By Lemma \ref{thetaparam}, for $j = 0, \dots, n-1$, we can fix 
\[
b_{n-1}^*(m_{n-1}, \theta) = e^{-\frac{1}{\rho}(m_{n-1}\pi - \theta)}
\]

and obtain unique $b_0^*(m_0, \theta), \dots, b_{n-2}^*(m_{n-2}, \theta)$ which solve $F(b_0^*(m_0, \theta), b_{n-1}^*(m_{n-1}, \theta)) = 0$.\\
...
\[
G(b_1(r; m_1, \theta), \dots, b_{n-2}(r; m_{n-2}, \theta), r) = 0
\]
Thus for each of our parameterizations we have found our periodic $n-$pulse.
\end{proof}
\end{theorem}
