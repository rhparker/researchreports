...
% lemma : persistence of zero on axis
...
% Lemma : zero set of G leading into z^* 
...
\begin{lemma}
For sufficiently small $r_m$, the zero level set of $G_(a_0, a_1, r_m)$ contains a smooth curve joining $c^*(n, r_m)$ to $z^*(n, r_m)$ for each $n \in \Z$. On this curve, $a_1$ is strictly decreasing from $c^*(n, r_m)$ to 0. Thus for each $n \in \Z$ there is a smooth function $a_0^*(n, a_1, r_m)$ defined for $a_1 \in (0, c^*(n, r_m)$ such that 
