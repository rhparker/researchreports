...
where we have left the superscript $+$ off of $V$ for ease of notation. Write this equation as $V' = AV + B(x)V$, where
...
\begin{equation}
|e^{A(y-x)}P^s_0| \leq K e^{-\alpha(y-x)} && 0 \leq x \leq y
\end{equation}

Since $q^+(0, \beta^+)(x)$ lies in the stable manifold, we have for $x \geq 0$

\begin{equation}
|B(x)| = |q^+(0, \beta^+)(x)| \leq K_1 e^{-\alpha x}
\end{equation}

for some constant $K_1 > 0$. 

thus $|B(x)| \rightarrow 0 as x \rightarrow \infty$. It follows from Lemma 3.4 in Palmer84 that 

\begin{align*}
|P^s_+(x; \beta^+) - P^s_0| &\rightarrow 0 \text{ as }
x \rightarrow \infty
\end{align*}

Since $P^u_+(x; \beta^+) = I - P^s_+(x; \beta^+)$ and $P^u_0 = I - P^s_0$, it follows that 

\begin{align*}
|P^u_+(x; \beta^+) - P^u_0| &\rightarrow 0 \text{ as }
x \rightarrow \infty
\end{align*}

The ``negative'' piece of the exponential dichotomy is similar.\\

This is great and all, but does not give us the estimate we want. Maybe we could redo the proof in Palmer84 using the fact that we have an exponential bound on $B(x)$? Or maybe someone else has done this?
