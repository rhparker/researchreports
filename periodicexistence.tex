...
In the previous section we showed how a 2-periodic solution bifurcates from a 1-periodic solution. For higher number of pulses, we expect that we could also come up with a bifurcation structure, but that it would be quite complicated. Rather than do that, which for now will be left as a subject for future research, we will show the existence of multipulses in the case where the period is large.\\

Recall that for a multipulse, by Lemma \ref{diagonalG}, we have to satisfy the equations
...
If $r_m = 0$, the zero set of each of these will look exactly like the one for the 2-pulse. However, for $r_m > 0$, the symmetry argument we used above will not hold, so we do not expect the pitchfork to persist.\\
...
The parameter $a_{n-1}$ will be our free parameter. For our length parameters, we choose a series of $n - 1$ nonnegative integers $m_0, \dots, m_{n-2}$ with the restriction that at least one of them must be 0.\\

Let $b^* = \min\{b^*_{m_0}, \dots, b^*_{m_{n-2}}\}$ and choose any $\epsilon$ with $0 < \epsilon < b^*$. Let $I = 0, b^* - \epsilon$. Then applying Lemma \ref{tubepersists} $n-1$ times, for $i = 0, \dots, n-1$ there exist $r_i > 0$ and a unique smooth function $h_i: I \times \mathcal{R} \cap [0, r_i] \rightarrow \R$ such that for all $r_m < r_i$, 
