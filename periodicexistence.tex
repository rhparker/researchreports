...
For $r_m$ sufficiently small, there exists a smooth function $p^*: \Z \times \mathcal{R} \rightarrow \R^+$ such that for all $n \in \Z$

\begin{equation}
p^*(n, r_m) \rightarrow e^{n \pi/\rho} \text{ as } r_m \rightarrow 0
\end{equation}
...
Thus, by the IFT, we can solve for $a_0$ in terms of $r_m$. In other words, we can find a unique family of functions (indexed by $n$) $a_0 = p^*(n, r_m)$ with the same smoothness as $G$ such that $p^*(n, 0) = e^{n \pi/\rho}$ and $G(p^*(n, r_m),0, r_m) = H(p^*(n, r_m), r_m) = 0$ for $r_m$ sufficiently small.

\end{proof}
\end{lemma}

We have shown the persistence of the pitchfork and of the zero on the $a_0$ axis for small $r_m$. The final thing we need to show is that the path connecting the two also persists for small $r_m$.
