...
for $i = 0, 1$. This was obtained via Lin's method and is the jump $\xi_i$ between the $i^\pm$ pieces in the direction of $\Psi(0)$. In other words,
...
In terms of the $X_i$, by Lemma \eqref{jumplemma1},

\begin{align*}
G_i(X_i, X_{i-1}) &= \langle \Psi(0), \Phi^u_+(0, X_i; \beta_i^+) a_i^+ \rangle
- \langle \Psi(0), \Phi^s_-(0, -X_{i-1}, \beta_i^-) a_{i-1}^- \rangle \\
&+ \int_{X_i}^0 \langle \Psi(0), \Phi_+^u(0, y; \beta_i^+) G_i^+(y, V_i^+(y),\beta_i^+) \rangle dy \\
&- \int_{-X_{i-1}}^0 \langle \Psi(0), \Phi_-^s(0, y, \beta_i^-) G_i^-(y, V_i^-(y),\beta_i^-) \rangle dy
\end{align*}

Suppose we have completed all but the final step of Lin's method, so have obtained everything we need to evaluate the above expression. Note that each inversion step in Lin's method gives a unique solution for the relevant variable.\\

In our setup, the four pieces are arranged $U_0^-, U_0^+, U_1^-, U_1^$, from L to R. By symmetry, if we replace $x$ by $-x$ and place the pieces in order $U_0^+, U_0^-, U_1^+, U_1^-$, we will have the exact same jumps. In precise terms, the jumps $\xi_i$ are also given by 

\begin{align*}
\xi_i &= \langle \Psi(0), \Phi^u_+(0, X_{i-1}; \beta_i^-) a_i^- \rangle
- \langle \Psi(0), \Phi^s_-(0, -X_{i-1}, \beta_i^-) a_{i-1}^- \rangle \\
&+ \int_{X_i}^0 \langle \Psi(0), \Phi_+^u(0, y; \beta_i^+) G_i^+(y, V_i^+(y),\beta_i^+) \rangle dy \\
&- \int_{-X_{i-1}}^0 \langle \Psi(0), \Phi_-^s(0, y, \beta_i^-) G_i^-(y, V_i^-(y),\beta_i^-) \rangle dy
\end{align*}
