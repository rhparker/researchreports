...
= &-\langle \psi(L), a_{i}^-\rangle\\
&-d_i \left( \int_{-\infty}^0 \langle \Psi(y), (K B Q_c)(y) \rangle dy + \int_0^\infty \langle \Psi(y), (K B Q_c)(y) \rangle dy \right) + d_i \int_L^\infty \langle \Psi(y), (K B Q_c)(y) \rangle dy \\
&+\left( \int_{-\infty}^0 \langle \Psi(y), G_i^- W_i^-(y) - d_i (K_i^- B (R_i^-)_c)(y) \rangle dy - \int_{L}^0 \langle \Psi(y), G_i^+ W_i^+(y) - d_i (K_i^+ B (R_i^+)_c)(y) \rangle dy \right) \\
&+ \lambda\left( \int_{-\infty}^0 \langle \Psi(y), (K_i^- B W_i^-)(y) \rangle dy - \int_{L}^0 \langle \Psi(y), (K_i^+ B W_i^+)(y) \rangle dy \right) \\
= &-\langle \psi(L), a_{i}^-\rangle + d_i \langle q, q_c \rangle_{L^2(\R)} + d_i \int_L^\infty \langle \Psi(y), (K B Q_c)(y) \rangle dy \\
&+\left( \int_{-\infty}^0 \langle \Psi(y), G_i^- W_i^-(y) - d_i(K_i^- B (R_i^-)_c)(y) \rangle dy - \int_{L}^0 \langle \Psi(y), G_i^+ W_i^+(y) - d_i (K_i^+ B (R_i^+)_c)(y) \rangle dy \right) \\
&+ \lambda\left( \int_{-\infty}^0 \langle \Psi(y), (K_i^- B W_i^-)(y) \rangle dy - \int_{L}^0 \langle \Psi(y), (K_i^+ B W_i^+)(y) \rangle dy \right) 
\end{align*}
...
Now we consider the other terms in the jump one at a time. For now, we will base this off the estimates in Sandstede (1998). If this ends up working, we will adapt the things I did for the bound on the integration operator in the exponentially weighted norm.
...
\item For the term involving $a$, we use (3.49) in Sandstede (1998). We might have to modify this slightly (because of integration operator), but will will just copy it here for now.
