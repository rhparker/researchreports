...
Numerics suggest that the $d_i$ are definitely not identical. (For the interaction eigenfunction of the first unstable double pulse, for example, one side is significantly larger than the other.) Recalling what we did in the previous section, we can write this as an expansion in $\lambda$ for $\lambda$ near 0.

\begin{equation}\label{dpeigform}
V_i^\pm = d_i( Q_2' + \lambda Q_{2c} ) + \lambda^2 W_i^\pm
\end{equation}
...
\begin{align*}
(d_i Q_2 &- d_i \lambda Q_{2c} + \lambda^2 W_i^\pm)' = \\
&d_i A(Q_2)Q_2' - d_i \lambda A(Q_2)Q_{2c} + \lambda^2 W_i^\pm + d_i \lambda KBQ_2' - d_i \lambda^2KBQ_{2c} + \lambda^3 K_i^\pm W_i^\pm 
\end{align*}
...
Using these expressions to cancel things and dividing by $\lambda^2$ (which we are assuming is nonzero since we know what happens when it is zero), we get

\[
(W_i^\pm)' = A(Q_2)W_i^\pm - d_i KBQ_{2c} + \lambda K_i^\pm BW_i^\pm
\]

This is exactly what we have in the one-pulse case, except we have replaced the single pulse with the double pulse. Of course, we could have just guessed this, but it is nice to show. But this is not what we want. We would like to write things in terms of the single pulse $Q$. Luckily, from Sandstede (1998) we know that
...
\begin{align*}
W_i^-(x) = \Phi^s_-(&x, -L_{i-1})a_{i-1}^- + \Phi^u_-(x, 0)b_i^- \\
&+ \int_0^x \Phi^u_-(x, y)[\lambda K_i^- BW_i^- - d_i KBQ_{c} + G_i^- W_i^- - d_i K_i^- B (R_i^-)_c ] dy \\
&+ \int_{-L_{i-1}}^x \Phi^s_-(x, y)[\lambda K_i^- BW_i^- - d_i KBQ_{c} + G_i^- W_i^- - d_i K_i^- B (R_i^-)_c ] dy \\
W_i^+(x) = \Phi^u_+(&x, L_i)a_i^+ + \Phi^s_+(x, 0)b_i^+ \\
&+ \int_0^x \Phi^s_+(x, y)[\lambda K_i^+ BW_i^+ - d_i KBQ_{c} + G_i^+ W_i^+ - d_i K_i+- B (R_i^+)_c ] dy \\
&+ \int_{L_i}^x \Phi^u_+(x, y)[\lambda K_i^+ BW_i^+ - d_i KBQ_{c} + G_i^+ W_i^+ - d_i K_i+- B (R_i^+)_c ]dy
\end{align*}

Next we write down the equation for the jump distance (evaluate at $x = 0)$. The following is analagous to (3.48) in Sandstede (1998). The $b_i$ cancel in that paper, so we will assume they cancel here too, although at some point we might want to look at that more carefully. In the meantime, we will just try and get this to work.

\begin{align*}
\langle \Psi(0), &W_i^-(0) - W_i^+(0) \rangle \\
= &\langle \psi(-L_{i-1}), a_{i-1}^-\rangle - \langle \psi(L_i), a_{i}^+\rangle\\
&-\left( \int_{-L_{i-1}}^0 \langle \Psi(y), d_i (K B Q_c)(y) \rangle dy - \int_{L_i}^0 \langle \Psi(y), d_i (K B Q_c)(y) \rangle dy \right) \\
&+\left( \int_{-L_{i-1}}^0 \langle \Psi(y), G_i^- W_i^-(y) - d_i (K_i^- B (R_i^-)_c)(y) \rangle dy - \int_{L_i}^0 \langle \Psi(y), G_i^+ W_i^+(y) - d_i (K_i^+ B (R_i^+)_c)(y) \rangle dy \right) \\
&+ \lambda\left( \int_{-L_{i-1}}^0 \langle \Psi(y), (K_i^- B W_i^-)(y) \rangle dy - \int_{L_i}^0 \langle \Psi(y), (K_i^+ B W_i^+)(y) \rangle dy \right) 
\end{align*}
...
&-d_1 \left( \int_{-\infty}^0 \langle \Psi(y), (K B Q_c)(y) \rangle dy + \int_0^\infty \langle \Psi(y), (K B Q_c)(y) \rangle dy \right) + d_1 \int_L^\infty \langle \Psi(y), (K B Q_c)(y) \rangle dy \\
&+\left( \int_{-\infty}^0 \langle \Psi(y), G_1^- W_1^-(y) - d_1 (K_1^- B (R_1^-)_c)(y) \rangle dy - \int_{L}^0 \langle \Psi(y), G_1^+ W_1^+(y) - d_1 (K_1^+ B (R_1^+)_c)(y) \rangle dy \right) \\
&+ \lambda\left( \int_{-\infty}^0 \langle \Psi(y), (K_i^- B W_i^-)(y) \rangle dy - \int_{L}^0 \langle \Psi(y), (K_i^+ B W_i^+)(y) \rangle dy \right) \\
= &-\langle \psi(L), a_{1}^+\rangle + d_1 \langle q, q_c \rangle_{L^2(\R)} + d_1 \int_L^\infty \langle \Psi(y), (K B Q_c)(y) \rangle dy \\
&+\left( \int_{-\infty}^0 \langle \Psi(y), G_1^- W_1^-(y) - d_1(K_1^- B (R_1^-)_c)(y) \rangle dy - \int_{L}^0 \langle \Psi(y), G_1^+ W_1^+(y) - d_1 (K_1^+ B (R_1^+)_c)(y) \rangle dy \right) \\
&+ \lambda\left( \int_{-\infty}^0 \langle \Psi(y), (K_1^- B W_1^-)(y) \rangle dy - \int_{L}^0 \langle \Psi(y), (K_1^+ B W_1^+)(y) \rangle dy \right) 
\end{align*}

where we used the Melnikov result from the single pulse case. Now all we have to do is hope the rest of the terms are sufficiently small.\\

Now we consider the other terms in the jump one at a time. For now, we will base this off the estimates in Sandstede (1998). This uses those estimates exactly, which may or may not be the case. If this ends up working, we will adapt the things I did for the bound on the integration operator in the exponentially weighted norm, which should have no effect other than modifying constants.
...
\end{align*}
where we used integration by parts to get the second line. This is a constant which depends only on the original single pulse $q$, so we will call it $C_1$.

\item For the term involving $a$, we use (3.49) in Sandstede (1998).

\begin{align*}
\langle \psi(L), a_1^+ \rangle &= \langle \psi(L), P_0^u D_1 d \rangle + \mathcal{O}(e^{-\alpha L}((e^{-\alpha L} + |G|)|D| + \lambda)|d|)
\end{align*}

To leading order, this should be

\begin{align*}
\langle \psi(L), Q'(-L) \rangle(d_2 - d_1)
\end{align*}

analogous to (3.56) in Sandstede (1998). 
...
\[
\langle \Psi(0), W_1^-(0) - W_1^+(0) \rangle = 
d_1 \langle q, q_c \rangle_{L^2(\R)} + \langle \psi(L), Q'(-L) \rangle(d_2 - d_1) + d_1 C_1 + (R(\lambda)d)_1
\]
...
This is again a constant, which we will call $C_2$. Thus we should have

\[
\langle \Psi(0), W_2^-(0) - W_2^+(0) \rangle = 
d_2 \langle q, q_c \rangle_{L^2(\R)} + \langle \psi(-L), Q'(L) \rangle(d_2 - d_1) + d_2 C_2 + (R(\lambda)d)_2
\]

Putting these together, we have to solve the following system for $(d_1, d_2)$.

\begin{align*}
d_1 \langle q, q_c \rangle_{L^2(\R)} + \langle \psi(L), Q'(-L) \rangle(d_2 - d_1) + d_1 C_1 + (R(\lambda)d)_1 = 0\\
d_2 \langle q, q_c \rangle_{L^2(\R)} + \langle \psi(-L), Q'(L) \rangle(d_2 - d_1) + d_2 C_2 + (R(\lambda)d)_2 = 0
\end{align*}
