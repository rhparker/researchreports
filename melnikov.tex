...
&\leq C \left[ (e^{-\alpha L} + |G| + |\lambda|) |\lambda|^2 + e^{\eta L}(|G| + |\lambda|)^2 |D_1| + e^{-\alpha L}(e^{-\alpha L} + |G| + |\lambda|) |D_1| \right] |d| \\
\end{align*}

Now define $D_1^{\tilde{\eta}}$ by

\[
D_1^{\tilde{\eta}} = e^{\tilde{\eta} L} D_1
\]
where we choose $0 < \eta < \tilde{\eta} \alpha$. We showed that $Q$, $Q'$, and $Q_c$ decay asymptotically exponentially with decay rate $\alpha$, thus $Q(L)$, $Q(-L)$, and their relevant derivatives are a constant multipled by $e^{-\alpha L}$, which gives us

\[
|D_1^{\tilde{\eta}}| \leq C e^{-(\alpha - \tilde{\eta})L}
\]

Thus we have

\begin{align*}
|D_1| &\leq e^{-\tilde{\eta} L} |D_1^{\tilde{\eta}}| \\
\end{align*}
...
\begin{align*}
|(R&(\lambda)d)_1| \\
&\leq  C \left[ (e^{-\alpha L} + |G| + |\lambda|) |\lambda|^2 + e^{\eta L}(|G| + |\lambda|)^2 e^{-\tilde{\eta} L} |D_1^{\tilde{\eta}}| + e^{-\alpha L}(e^{-\alpha L} + |G| + |\lambda|) e^{-\tilde{\eta} L} |D_1^{\tilde{\eta}}|\right] |d| \\
&\leq  C \left[ (e^{-\alpha L} + |G| + |\lambda|) |\lambda|^2 + e^{\eta L}(|G| + |\lambda|)^2 e^{-\tilde{\eta} L} |D_1^{\tilde{\eta}}| + e^{-\alpha L}(e^{-\alpha L} + |G| + |\lambda|) e^{-\tilde{\eta} L} |D_1^{\tilde{\eta}}|\right] |d| \\
\end{align*}


If we take $|\lambda|$ and $|G|$ of order $e^{-\alpha L}$, the overall order of the remainder is
