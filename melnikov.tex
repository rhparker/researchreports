...
\end{equation}
where we can take the constant of integration to be 0 since the pulse decays to 0 on both ends. If we write \eqref{4thorder} as a first-order system and linearize about the equilibrium solution at 0, we obtain the constant-coefficient matrix system $U' = A(0) U$, where
...
For $c < 1/4$, all four of these are real (two positive and two negative), and for $c > 1/4$, we get two complex conjugate pairs (one with positive real part and one with negative real part). In both cases, we have a two-dimensional stable eigenspace and a two-dimensional unstable eigenspace. We will need the two stable eigenvalues, so we designate them by $\nu_1^s(c)$ and $\nu_2^s(c)$, where
...
The corresponding eigenvectors are $v_1^s(c)$ and $v_2^s(c)$. The unstable eigenvalues are $\nu_1^u(c)$ and $\nu_2^u(c)$, with corresponding eigenvectors $v_1^u(c)$ and $v_2^u(c)$. The stable and unstable eigenspaces are $E_s(c) = \text{span}\{ v_1^s(c), v_2^s(c) \}$ and $E_u(c) = \text{span}\{ v_1^u(c), v_2^u(c) \}$
...
\begin{proof}
Recall that near the equilibrium at 0, the stable manifold $W^s(0)$ is given by $\{ (u, h(u)) : u \in E_s(c) \text{near} 0 \}$ and $h: E_s(c) \rightarrow E_u(c)$ is smooth (should be $C^\infty$ since our nonlinearity is a simple quadratic nonlinearity). From the construction of the single pulse $q(x, c)$, we know that it lies on the stable manifold (as $x \rightarrow \infty$). Thus for large $x$, we can write $q(x; c)$ as
\begin{equation}
q(x; c) = q_1(x; c) v_1^s(c) + q_2(x; c) v_2^s(c) + h( q_1(x; c), q_2(x; c) )
\end{equation}
By the stable manifold theorem, $q(x; c) \rightarrow 0$ exponentially as $x \rightarrow \infty$. In other words, $q_1(x; c)$ and $q_2(x; c)$ go to 0 exponentially as $x \rightarrow \infty$ with decay rates given by the real parts of the eigenvalues $\nu_1^s(c)$ and $\nu_2^s(c)$ (these are negative). Since $h$ is smooth (Lipschitz continuity is enough here, I think), then that term goes to 0 exponentially as well.\\

We want to look at the derivative with respect to $c$. First, let's take the last term on the RHS. Writing $h$ as $h(s, t)$:
\begin{align}
...
\begin{enumerate}
	\item $q(x; c)$ is continuous in $c$. This should follow from continuous dependence on parameters.
	\item $\tilde{q}_1(x; c)$ is continuous in $c$.
	\item Given $c_0 > 0$, we can find an $M > 0$, an $R > 0$, and a $\delta > 0$ so that for all $x \geq M$, $|\tilde{q}_1(x; c)| \leq R$ for all $x \geq M$ and for all $c \in (c_0 - \delta, c_0 + \delta)$.
\end{enumerate}

So what we need to show is that $\frac{\partial}{\partial_c}q_1(x; c)$ at $c = c_0$ decays exponentially.
...
If we plug this into the integrated eigenvalue problem, we obtain the following system of equations, which are analagous to (3.7) in Sandstede (1998).
...
In particular, note that compared to the double pulse (see \textrm{KdV17}), the boundary term is gone, and there is no longer a match at $\pm L$. To solve this, we have the equivalent of Lemmas 3.3 - 3.5 in Sandstede (1998).
...
We also define the linear operators $(L_2(\lambda)b)(x)$ from $\R^n$ to our exponentially weighted space (TECHNICALLY THE RANGE OF THIS IS THE NON-EXPONENTIALLY WEIGHTED SPACE, BUT SINCE EVERYTHING WE ARE DEALING WITH IN THIS OPERATOR IS EXPONENTIALLY LOCALIZED, ITS OUTPUT SHOULD BE IN THE EXPONENTIALLY WEIGHTED SPACE) by
...
We are interested in the terms not involving $\lambda$. (If we recall what we did above, these will correspond to the coefficient of $\lambda^2$ in the expansion \eqref{eigenfntosecondorder} of the eigenfunction.) Recall that we defined $\psi$ as the 4th component of $\Psi$, and showed above that $\psi \in \ker H$. By Assumption \ref{1dkernel}, $\psi = q'$, so we have for the term not involving $\lambda$

\begin{align*}
M &= \int_{-\infty}^0 \psi(y) \int_{-\infty}^y q_c(z) dz dy + \int_0^\infty \psi(y) \int_{\infty}^y q_c(z) dz dy \\
&= \int_{-\infty}^0 q'(y) \int_{-\infty}^y q_c(z) dz dy + \int_0^\infty q'(y) \int_{\infty}^y q_c(z) dz dy
\end{align*}

We wish to show that $M$ (the analogue of the Melnikov integral) is nonzero. First, we define the piecewise function
\[
...
\[
M = \int_{-\infty}^0 q'(y) \tilde{q}(y) dy + \int_0^\infty q'(y) \tilde{q}(y) dy = \int_{-\infty}^\infty q'(y) \tilde{q}(y) dy = \langle q', \tilde{q} \rangle
\]

Now we integrate by parts. Noting that $\partial_x \tilde{q} = q_c$, we get

\begin{align*}
M = \langle q', \tilde{q} \rangle &= -\langle q, \partial_x \tilde{q} \rangle = -\langle q, q_c \rangle
\end{align*}
which is nonzero by Assumption \ref{qcIP}. SINCE $\tilde{q}$ FAILS TO BE DIFFERENTIABLE ONLY AT A SINGLE POINT, THIS SHOULD BE OK.
