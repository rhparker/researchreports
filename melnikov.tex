...
\begin{equation}\label{KBW}
\left| \int_{-\infty}^0 \langle \Psi(y), (K_1^- B W_1^-)(y) \rangle dy - \int_{L}^0 \langle \Psi(y), (K_1^+ B W_1^+)(y) \rangle dy \right| \leq C\left( ( 1 + e^{\eta L}(|G| + |\lambda|)) |D_1| + |\lambda|^2 \right) |d|
\end{equation}
...
&+ C |\lambda|\left(( 1 + e^{\eta L}(|G| + |\lambda|)) |D_1| + |\lambda|^2 \right) |d| \\
&+ C |\lambda^2| e^{-\alpha L} |d| \\
&\leq C \left[ (e^{-\alpha L} + |G| + |\lambda|) |\lambda|^2 + e^{\eta L}(|G| + |\lambda|)^2 |D_1| + e^{-\alpha L}(|G| + |\lambda|) |D_1| + |\lambda||D_1| \right] |d| \\
\end{align*}
...
\begin{align*}
|(R&(\lambda)d)_1|  \\
&\leq C \left[ (e^{-\alpha L} + |G| + |\lambda|) |\lambda|^2 + e^{\eta L}(|G| + |\lambda|)^2 e^{-\eta L} |D_1^{\eta}| + e^{-\alpha L}(|G| + |\lambda|) e^{-\eta L} |D_1^{\eta}| + |\lambda|e^{-\eta L} |D_1^{\eta}| \right] |d| \\
&= C \left[ (e^{-\alpha L} + |G| + |\lambda|) |\lambda|^2 + (|G| + |\lambda|)^2 |D_1^{\eta}| + e^{-(\alpha+\eta) L}(|G| + |\lambda|)  |D_1^{\eta}| + |\lambda|e^{-\eta L} |D_1^{\eta}| \right] |d| \\
\end{align*}
...
\begin{align*}
|(R&(\lambda)d)_1|  \\
&\leq C \left[ e^{-3 \alpha L} + e^{-2 \alpha L} |D_1^{\eta}| + e^{-(2\alpha+\eta) L} |D_1^{\eta}| + e^{-(\alpha + \eta) L} |D_1^{\eta}| \right] |d| \\
&\leq C \left[ e^{-3 \alpha L} + e^{-2 \alpha L} e^{-(\alpha - \eta)L} + e^{-(2\alpha+\eta) L} e^{-(\alpha - \eta)L} + e^{-(\alpha + \eta) L} e^{-(\alpha - \eta)L} \right] |d| \\
&= C \left[ e^{-3 \alpha L} + e^{-(3 \alpha - \eta)L} + e^{-2\alpha L} \right] |d| 
\end{align*}

THE LAST TERM ON THE RHS IS NOT HIGH ENOUGH ORDER (THE NONREMAINDER TERMS ARE ORDER $e^{-2 \alpha L}$< SO WE NEED REMAINDER TO HAVE A HIGHER ORDER THAN THIS.) THIS TERM IN THE ESTIMATE COMES FROM EQUATION \eqref{KBW}. I DO NOT THINK THE REST WILL WORK UNLESS WE CAN IMPROVE THIS ESTIMATE. OTHER THAN THAT THIS APPEARS TO WORK.\\
