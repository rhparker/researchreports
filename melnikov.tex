...
\[
\left| \int_{-\infty}^0 \langle \Psi(y), (K_1^- B W_1^-)(y) \rangle dy - \int_{L}^0 \langle \Psi(y), (K_1^+ B W_1^+)(y) \rangle dy \right| \leq C\left(\left( e^{-\eta L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d|
\]
...
\begin{align*}
|(R&(\lambda)d)_1| \leq C |\lambda^2| |d| e^{-\alpha L} + C \left( e^{-(\alpha +\eta) L}(|\lambda|^2 + (e^{-\alpha L} + |G| + |\lambda|)|D_1^\eta|)|d| \right) \\
&+ C |G| \left( \left( e^{-(\alpha + \eta) L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right)|d| \\
&+ C |\lambda|\left(\left( e^{-\eta L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right) |d|\\
&+ C |\lambda^2| e^{-\alpha L} |d| \\
&\leq C \left[ (e^{-\alpha L} + |G| + |\lambda|) |\lambda|^2 + (e^{-(\alpha + \eta)L} + |G|)(e^{-\alpha L} + |G| + |\lambda|)|D_1^\eta| + |\lambda| (e^{-\eta L} + |G| + |\lambda|)|D_1^\eta| \right]|d|\\
\end{align*}
...
\begin{align*}
|(R(\lambda)d)_1| &\leq C [ (e^{-\alpha L} + |G| + |\lambda|) |\lambda|^2 + (e^{-(\alpha + \eta)L} + |G|)(e^{-\alpha L} + |G| + |\lambda|)e^{-(\alpha - \eta)L} \\
&+ |\lambda| (e^{-\eta L} + |G| + |\lambda|) e^{-(\alpha - \eta)L} ] |d| \\
&=C [ (e^{-\alpha L} + |G| + |\lambda|) |\lambda|^2 + (e^{-(\alpha + \eta)L} + |G|)(e^{-\alpha L} + |G| + |\lambda|)e^{-(\alpha - \eta)L} \\
&+ |\lambda| (e^{-\eta L} + |G| + |\lambda|) e^{-(\alpha - \eta)L} ] |d|
\end{align*}
