...
In other words, this makes sense only if $\lambda = 0$ or the other term in the RHS is 0. In the latter case, we will say that the mean zero condition for $W^\pm$ is satisfied. So for $\lambda \neq 0$, the eigenvalue problem and the integrated eigenvalue problem are equivalent if and only if the mean zero condition is satisfied. I have no idea how (or if) we can guarantee this condition, but it will be essential to what follows.\\
...
At this point, we can plug in our expressions for $W^\pm$ from above and see what we get. Unfortunately, this is a really big mess, so we will approach this from another angle. From what we have proved, we know that $W^\pm$ is analytic in $\lambda$, so we can expand it as a Taylor series in $\lambda$ about $\lambda = 0$. The constant term in the Taylor expansion must be zero since we know what happens when $\lambda = 0$. Thus we have
...
\[
W(\lambda, x)^\pm = -Q_c(x) + \mathcal{O}(\lambda^2)
\]
...
\rangle\\
&= \begin{pmatrix}q_c(0)\\ 0\\ q_{cxx}(0)\\0\end{pmatrix}
\begin{pmatrix}0\\ q_{xx}(0)\\ 0\\q_{xxxx}(0)\end{pmatrix}\\
&= 0
\end{align*}
...
&= -\lambda^2 \left( \int_{-\infty}^0 \langle \Psi(y), (K^- B Q_c)(y) \rangle dy + \int_0^\infty \langle \Psi(y), (K^+ B Q_c)(y) \rangle dy \right) \\
&+ \lambda^3 \left( \int_{-\infty}^0 \langle \Psi(y), (K^- B \tilde{W}^-)(y) \rangle dy + \int_0^\infty \langle \Psi(y), \lambda (K^+ B \tilde{W}^+)(y) \rangle dy \right) 
\end{align*}

We are only concerned with the coefficient of $\lambda^2$. Since we know that the 4th component of $\Psi$ is $q'$, this becomes (upon integration by parts)

\begin{align*}
\int_{-\infty}^0 &\langle \Psi(y), (K^- B Q_c)(y) \rangle dy + \int_0^\infty \langle \Psi(y), (K^+ B Q_c)(y) \rangle dy\\
&= \int_{-\infty}^0 q'(y) \int_{-\infty}^y q_c(z) dzdy + \int_0^\infty q'(y) \int_{\infty}^y q_c(z) dz dy\\
&= q(y) \int_{-\infty}^y q_c(z) dz \Big|_{-\infty}^0 - \int_{-\infty}^0 q(y)q_c(y) dy + q(y) \int_{\infty}^y q_c(z) dz \Big|_0^\infty - \int_0^\infty q(y)q_c(y) dy\\
&= q(0) \left( \int_{-\infty}^0 q_c(z) dz - \int_{\infty}^0 q_c(z) dz \right) - \int_{-\infty}^\infty q(y) q_c(y) dy\\
&= q(0) \int_{-\infty}^\infty q_c(z) dz - \int_{-\infty}^\infty q(y) q_c(y) dy
\end{align*}

The second term on the RHS is the higher order Melnikov integral that we want. The first term on the RHS is the annoying boundary term which we got before when we did this the other way. I am not sure what to do from here.
