...
Numerics suggest that the $d_i$ are definitely not identical. (For the interaction eigenfunction of the first unstable double pulse, for example, one side is significantly larger than the other.) Since $d_i$ is nonzero, there is no reason we cannot have it multiply the $W_i^pm$.

\[
V_i^\pm = d_i (Q_2' + W_i^\pm)
\]

Finally, recalling what we did in the previous section, we can write this as an expansion in $\lambda$ for $\lambda$ near 0.

\begin{equation}\label{dpeigform}
V_i^\pm = d_i( Q_2' + \lambda Q_{2c} + \lambda^2 W_i^\pm)
\end{equation}

The perturbation $W_i^\pm$ is different in each case. When $\lambda = 0$, we can take $d_i = 1$ to get the correct eigenfunction $Q_2'$, so that is good. Also we will eventually want this in terms of the single pulse $Q$, but we leave it this for now to get some nice cancelation. Recall that the integrated eigenvalue problem is given in piecewise system form by
...
\begin{align*}
(Q_2 &- \lambda Q_{2c} + \lambda^2 W_i^\pm)' = \\
&A(Q_2)Q_2' - \lambda A(Q_2)Q_{2c} + \lambda^2 W_i^\pm + \lambda KBQ_2' - \lambda^2KBQ_{2c} + \lambda^3 K_i^\pm W_i^\pm 
\end{align*}
...
\[
(W_i^\pm)' = A(Q_2)W_i^\pm- KBQ_{2c} + \lambda K_i^\pm BW_i^\pm
\]
...
(W_i^\pm)' = A(Q)W_i^\pm - KBQ_{c} + \lambda K_i^\pm BW_i^\pm + G_i^\pm W_i^\pm - K_i^\pm B (R_i^\pm)_c
\end{equation}  
...
\begin{align*}
W_i^-(x) = \Phi^s_-(&x, -L_{i-1})a_i^- + \Phi^u_-(x, 0)b_i^- \\
&+ \int_0^x \Phi^u_-(x, y)[\lambda K_i^- BW_i^- - KBQ_{c} + G_i^- W_i^- - K_i^- B (R_i^-)_c ] dy \\
&+ \int_{-L_{i-1}}^x \Phi^s_-(x, y)[\lambda K_i^- BW_i^- - KBQ_{c} + G_i^- W_i^- - K_i^- B (R_i^-)_c ] dy \\
W_i^+(x) = \Phi^u_+(&x, L_i)a_i^+ + \Phi^s_+(x, 0)b_i^+ \\
&+ \int_0^x \Phi^s_+(x, y)[\lambda K_i^+ BW_i^+ - KBQ_{c} + G_i^+ W_i^+ - K_i+- B (R_i^+)_c ] dy \\
&+ \int_{L_i}^x \Phi^u_+(x, y)[\lambda K_i^+ BW_i^+ - KBQ_{c} + G_i^+ W_i^+ - K_i+- B (R_i^+)_c ]dy
\end{align*}
...
= &\Phi^s_-(0, -L_{i-1})a_i^- -\Phi^u_+(0, L_i)a_i^+\\
&-\left( \int_{-L_{i-1}}^0 \langle \Psi(y), (K B Q_c)(y) \rangle dy - \int_{L_i}^0 \langle \Psi(y), (K B Q_c)(y) \rangle dy \right) \\
&+\left( \int_{-L_{i-1}}^0 \langle \Psi(y), G_i^- W_i^-(y) - (K_i^- B (R_i^-)_c)(y) \rangle dy - \int_{L_i}^0 \langle \Psi(y), G_i^+ W_i^+(y) - (K_i^+ B (R_i^+)_c)(y) \rangle dy \right) \\
...
= &\Phi^s_-(0, -\infty)a_i^- -\Phi^u_+(0, L)a_i^+\\
&-\left( \int_{-\infty}^0 \langle \Psi(y), (K B Q_c)(y) \rangle dy + \int_0^\infty \langle \Psi(y), (K B Q_c)(y) \rangle dy \right) + \int_L^\infty \langle \Psi(y), (K B Q_c)(y) \rangle dy \\
&+\left( \int_{-\infty}^0 \langle \Psi(y), G_i^- W_i^-(y) - (K_i^- B (R_i^-)_c)(y) \rangle dy - \int_{L}^0 \langle \Psi(y), G_i^+ W_i^+(y) - (K_i^+ B (R_i^+)_c)(y) \rangle dy \right) \\
&+ \lambda\left( \int_{-\infty}^0 \langle \Psi(y), (K_i^- B W_i^-)(y) \rangle dy - \int_{L}^0 \langle \Psi(y), (K_i^+ B W_i^+)(y) \rangle dy \right) \\
= &\Phi^s_-(0, -\infty)a_i^- -\Phi^u_+(0, L)a_i^+ + \langle q, q_c \rangle_{L^2(\R)} + \int_L^\infty \langle \Psi(y), (K B Q_c)(y) \rangle dy \\
&+\left( \int_{-\infty}^0 \langle \Psi(y), G_i^- W_i^-(y) - (K_i^- B (R_i^-)_c)(y) \rangle dy - \int_{L}^0 \langle \Psi(y), G_i^+ W_i^+(y) - (K_i^+ B (R_i^+)_c)(y) \rangle dy \right) \\
&+ \lambda\left( \int_{-\infty}^0 \langle \Psi(y), (K_i^- B W_i^-)(y) \rangle dy - \int_{L}^0 \langle \Psi(y), (K_i^+ B W_i^+)(y) \rangle dy \right) 
\end{align*}
...
Now we consider the other terms in the jump one at a time.
...
\end{align*}
where we used integration by parts to get the second line.
