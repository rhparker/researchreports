...
where $K^\pm$ is the integration operator

\[
(K^\pm F)(x) = \int_{\pm \infty}^x F(y) dy
\]

Note that for the above problem to make sense, we need the RHS of the $W^\pm$ equation in \eqref{inteig1} to be continuous at $x = 0$. Suppose we have found a solution to \eqref{inteig1} such that $W^-(0) = W^+(0)$. Then since $Q(x)$ is continuous, the RHS matches at 0 if and only if 

\begin{align*}
0 &= \lambda (K^- B W^-)(0) - \lambda (K^- B W^+)(0) \\
&= \lambda \left( \int_{-\infty}^0 BW^-(y) dy + \int_0^{\infty} BW^+(y) dy\right)
\end{align*}

In other words, this makes sense only if $\lambda = 0$ or the term in parentheses on the RHS is 0. In the latter case, we will say that the mean zero condition for $W^\pm$ is satisfied. \\

Thus for $\lambda \neq 0$, if we write $V^\pm(x) = Q'(x) + W^\pm(x)$, $W^\pm(x)$ solves the eigenvalue problem if and only if $W^\pm(x)$ solves the integrated eigenvalue problem and the mean-zero condition is satisfied.\\

I have no idea how (or if) we can obtain the mean-zero condition, but it will be essential to what follows.\\

We will use the third equation in \eqref{inteig1} together with the jump distance to conclude that $W^-(0) = W^+(0)$, which is the matching condition we want.\\
...
Writing the fixed point problem as $(I - L_1(\lambda))W = L_2(\lambda)(b)$, we can invert the operator ($(I - L_1(\lambda))$ to get
\[
...
&- \langle \Psi(0), \Phi^s_+(0, 0)b^+ \rangle - \int_\infty^0 \Phi^u_+(0, y)[\lambda (K^+ B W^+)(y) + \lambda B Q(y) ] dy \rangle\\
...
&+ \int_0^\infty \langle \Psi(y), \lambda (K^+ B W^+)(y) + \lambda B Q(y)  \rangle dy \\
&= \lambda\left( \int_{-\infty}^0 \langle \Psi(y), (K^- B W^-)(y) \rangle dy + \int_0^\infty \langle \Psi(y), \lambda (K^+ B W^+)(y) \rangle dy \right) \\
&+ \lambda \int_{-\infty}^\infty \langle\Psi(y), BQ(y) \rangle dy
\end{align*}

where we terms involving the $b^\pm$ are zero since $b^\pm \perp \Psi(0)$.\\

The final integral on the RHS is the standard Melnikov integral, which we claim is 0. To see this, we first note that for the linearization of the fourth-order equation about the single pulse $q$, i.e. $H = \partial_x^4 - \partial_x^2 + c - 2q$, $Hq_c = -q$. (To show this, plug it in and use the fact that $q$ solves the orginal nonlinear 4th order equation.) Using this and the definition of $B$ (see \texttt{KdV17}) we have
...
\[
\langle \Psi(0), W^+(0) - W^-(0) \rangle = \lambda\left( \int_{-\infty}^0 \langle \Psi(y), (K^- B W^-)(y) \rangle dy + \int_0^\infty \langle \Psi(y), \lambda (K^+ B W^+)(y) \rangle dy \right) 
\]

At this point, we can plug in our expressions for $W^\pm$ from above and see what we get. Unfortunately, this is a really big mess, so we will approach this from another angle. \\

Before we go any further, we will integrate by parts on the RHS.

\begin{align*}
\langle \Psi(0), &W^+(0) - W^-(0) \rangle = \lambda\left( \int_{-\infty}^0 q'(x) \int_{-\infty}^x w^-(y) dy dx + \int_0^{\infty} q'(x) \int_{\infty}^x w^+(y) dy dx \right) \\
&= \lambda \left( \int_{-\infty}^0 q'(y) \int_{-\infty}^y w^-(z) dzdy + \int_0^\infty q'(y) \int_{\infty}^y w^+(z) dz dy \right)\\
&= \lambda \left( q(y) \int_{-\infty}^y w^-(z) dz \Big|_{-\infty}^0 - \int_{-\infty}^0 q(y)w^-(y) dy + q(y) \int_{\infty}^y w^+(z) dz \Big|_0^\infty - \int_0^\infty q(y)w^+(y) dy \right) \\
&= \lambda \left( q(0) \int_{-\infty}^0 w^-(z) dz - q(0) \int_{\infty}^0 w^+(z) dz \right) - \lambda \left( \int_{-\infty}^0 q(y)w^-(y) dy +  \int_0^\infty q(y)w^+(y) dy \right) \\
&= \lambda q(0) \left( \int_{-\infty}^0 w^-(z) dz + \int_0^\infty w^+(z) dz \right) - \lambda \left( \int_{-\infty}^0 q(y)w^-(y) dy +  \int_0^\infty q(y)w^+(y) dy \right) \\
&= \lambda q(0) \left( \int_{-\infty}^0 BW^-(z) dz + \int_0^\infty BW^+(z) dz \right) - \lambda \left( \int_{-\infty}^0 q(y)w^-(y) dy +  \int_0^\infty q(y)w^+(y) dy \right) \\
\end{align*}

The first term in parentheses on the RHS is the mean-zero condition. Since for $\lambda \neq 0$ we need this satisfied for the integrated eigenvalue problem to make sense and to imply a solution to the eigenvalue problem, we will assume for now that we have somehow satisfied that criterion. NOTE THAT WE HAVE NO IDEA AT PRESENT HOW WE WILL DO THAT. In that case, this becomes

\begin{align*}
\langle \Psi(0), &W^+(0) - W^-(0) \rangle = - \lambda \left( \int_{-\infty}^0 q(y)w^-(y) dy +  \int_0^\infty q(y)w^+(y) dy \right) \\
\end{align*}

Now we look for an expansion for $W^\pm$. From what we have proved, we know that $W^\pm$ is analytic in $\lambda$, so we can expand it as a Taylor series in $\lambda$ about $\lambda = 0$. The constant term in the Taylor expansion must be zero since we know what happens when $\lambda = 0$. Thus we have
...
\begin{align*}
\lambda (V^\pm)'(x) &= A(q(x)) \lambda V^\pm(x) + \lambda B Q(x) + \lambda^2 K^\pm B V^\pm(x) 
\end{align*}

Since we are interested in $\lambda \neq 0$ and we know what happens when $\lambda = 0$, we can divide by $\lambda$ to get

\begin{align*}
(V^\pm)'(x) &= A(q(x))V^\pm(x) + B Q(x) + \lambda K^\pm B V^\pm(x) 
\end{align*}

To find $V^\pm(x)$, we set $\lambda = 0$ to cancel the last term on the RHS to get

\begin{align*}
(V^\pm)'(x) &= A(q(x))V^\pm(x) + B Q(x) 
\end{align*}

which we have shown has unique solution $V^\pm(x) = -Q_c(x)$. In this case, $-Q_c(x)$ is continuous, so the two pieces $V^\pm(x)$ are automatically connected at $x = 0$. So we have written $W^\pm$ as

\[
W(\lambda, x)^\pm = -\lambda Q_c(x) + \mathcal{O}(\lambda^2)
\]
...
In other words, we need to show that $W^\pm(0) \perp Q'(0)$ for the expression we have for $W^\pm$. For this to be true, it must hold for the lowest order term, i.e. we need to show that $Q_c(0) \perp Q'(0)$. To do this, we take the standard $\R^4$ inner product to get

\begin{align*}
\langle Q_c(0), Q'(0) \rangle &= 
\langle 
\begin{pmatrix}q_c(0)\\ q_{cx}(0)\\ q_{cxx}(0)\\q_{cxxx}(0)\end{pmatrix}
\begin{pmatrix}q_x(0)\\ q_{xx}(0)\\ q_{xxx}(0)\\q_{xxxx}(0)\end{pmatrix}
\rangle\\
&= \langle \begin{pmatrix}q_c(0)\\ 0\\ q_{cxx}(0)\\0\end{pmatrix}
\begin{pmatrix}0\\ q_{xx}(0)\\ 0\\q_{xxxx}(0)\end{pmatrix} \rangle \\
&= 0
\end{align*}
...
For the leading order term $V^\pm = -Q_c$ this becomes

\[
Q_c^+(0) - Q_c^-(0) \in \C \psi(0) 
\]

which is trivially true since $Q_c$ is continuous.\\

Having verified these conditions, we can write

\[
W^\pm(x) = -\lambda Q_c(x) + \lambda^2 \tilde{W}^\pm(x) 
\]

Plugging this in to our jump equation, we have

\begin{align*}
\langle \Psi(0), &W^+(0) - W^-(0) \rangle = - \lambda \left( \int_{-\infty}^0 q(y)(-\lambda q_c(y) +\lambda^2 \tilde{w}^+(y)) dy + \int_0^\infty q(y)(-\lambda q_c(y) +\lambda^2 \tilde{w}^-(y)) dy \right) \\ 
&= \lambda^2 \int_{-\infty}^\infty q(y) q_c(y) dy - \lambda^3 \left( \int_{-\infty}^0 q(y)\tilde{w}^-(y) dy + \int_0^\infty q(y) \tilde{w}^+(y) dy \right) \\
&= \lambda^2 \langle q, q_c \rangle_{L^2(\R)} - \lambda^3 \left( \int_{-\infty}^0 q(y)\tilde{w}^-(y) dy + \int_0^\infty q(y) \tilde{w}^+(y) dy \right) 
\end{align*}

The coefficient of $\lambda^2$ on the RHS is the higher order Melnikov integral that we want. This agrees with Pego and Weinstein, as well as our intuition. By our hypothesis, this coefficient is nonzero. The other term on the RHS is higher order, and we should be able to handle it with our estimates.
