...
\begin{align*}
\langle &\Psi(0), W_1^-(0) - W_1^+(0) \rangle = d_1 \lambda^2 \langle q, q_c \rangle_{L^2(\R)} + \lambda S_1 + \langle \psi(L), Q'(-L) \rangle(d_2 - d_1) + (R(\lambda)d)_1
\end{align*}
...
For the second piece, most is similar to the first piece. We will do a few of the terms here

\begin{align*}
\langle \psi(-L), a_2^- \rangle &= -\langle \psi(-L), P_0^s D_1 d \rangle + \mathcal{O} \left( e^{-\alpha L} (e^{-\eta L}|\lambda|^2 + (e^{-\alpha L} + |G| + |\lambda|)|D_1|)|d| \right)\\
\end{align*}
...
\begin{align*}
\langle \Psi(-L), a_2^- \rangle = -\langle \Psi(-L), &Q'(L) \rangle (d_2 - d_1) - \lambda \langle \Psi(L), P_0^s (Q_c(-L) + Q_c(L))\rangle(d_2 - d_1) \\
&+ \mathcal{O} \left( e^{-\alpha L} (e^{-\eta L}|\lambda|^2 + (e^{-\alpha L} + |G| + |\lambda|)|D_1|)|d| \right)\\
\end{align*}
...
&= S_2 \lambda + \mathcal{O} \left[ \left(\left( e^{\eta L}(e^{-\alpha L} + |G| + |\lambda|) \right)|D_1| + |\lambda|^2 \right)|\lambda| |d|\right]
\end{align*}

where

\[
S_2 = \int_{-L}^0 \langle \Psi(x), \int_{-L}^x B \Phi^s_-(y, -L) P_0^s D_1 d  \:dy \rangle dxdx 
\]
...
d_2 \lambda^2 \langle q, q_c \rangle_{L^2(\R)} + S_2 \lambda + \langle \psi(-L), Q'(L) \rangle(d_2 - d_1) + (R(\lambda)d)_2
\]
...
Since this is now the same form as for a generic eigenvector, we conclude that for $\tilde{d} = (-d_2, -d_1)$, $S(-\lambda)\tilde{d} = 0$.
