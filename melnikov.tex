...
We consider the other terms in the jump one at a time, using the estimates we have found.
...
\begin{align*}
&\left| \int_{-\infty}^0 \langle \Psi(y), G_1^- W_1^-(y) \rangle dy - \int_L^0 \langle \Psi(y), G_1^- W_1^-(y) \rangle dy\right| \leq C |G| \left( \left( e^{-\alpha L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right)|d|
\end{align*}
...
\item For the terms involving the integral of the single pulse remainder $K_1^\pm B (R_1^\pm)_c$ we know that the remainder is in the weighted exponential space $C_0^\alpha$, so we have

\begin{align*}
\left| \int_{-\infty}^0 \langle \Psi(y), (K_1^- B (R_1^-)_c)(y) \rangle dy \right| &\leq
C \int_{-\infty}^0 e^{\alpha y} \int_{-\infty}^y e^{\alpha z} |e^{-\alpha z} r_{1c}^-(z)| dz dy \\
&\leq C ||R_{1c}^-||_\alpha \int_{-\infty}^0 e^{\alpha y} \int_{-\infty}^y e^{\alpha z} dz dy \\
&= C \frac{||R_{1c}^-||_\alpha}{\alpha} \int_{-\infty}^0 
\end{align*}
...
\begin{align*}
|(R(\lambda)d)_1| &\leq C \left( e^{-(\alpha +\eta) L}(|\lambda|^2 + (e^{-\alpha L} + |G| + |\lambda|)|D_1^\eta|)|d| \right) \\
&+ C |G| \left( \left( e^{-\alpha L} + |G| + |\lambda|  \right) |D_1^{\eta}| + |\lambda|^2 \right)|d| + C |\lambda| \left( |\lambda|^2 + |D_1^\eta|\right)|d| \\
&\leq C \left[ (e^{-(\alpha + \eta)L} + |G|) |\lambda|^2 + (e^{-(\alpha + \eta)L} + |G|)(e^{-\alpha L} + |G| + |\lambda|)|D_1^\eta| + |\lambda|^3 + |\lambda| |D_1^\eta| \right]|d|
\end{align*}
