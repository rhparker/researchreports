...
& \leq C \Big( (p_1(X_i; \lambda) + |G|)|a_i| + |G||a_{i-1}| + (e^{-\alpha X_i} + |G|) |b| + ( p_2(X_i; \lambda) + e^{\nu(\lambda)X_M} |G|) |c^+| \\
&+ (1 + e^{\nu(\lambda)X_i} |G|)|c_i^-| + e^{\nu(\lambda)X_{i-1}} |G||c_{i-1}^-| + (e^{-\tilde{\alpha} X_i} + |G|) |\lambda|^2 |d| \Big)
\end{align*} 

At this point we will take $X_2 \geq X_1$ without loss of generality. We can do this since it does not matter where we place the origin. Thus we will have $X_M = X_2$ and $X_m = X_1$. This then becomes
...
At this point, we have a small complication. Since we have (or are assuming!) that $|G|$ is order $e^{-\alpha X_m} = e^{-\alpha X_1}$, we will have a term which is order $e^{\nu(\lambda)X_2} e^{-\alpha X_1}$, and there is no telling what this going to do as we increase our two length parameters. The simplest way to handle this is to have $X_2$ depend on $X_1$, so let's do the easiest thing and let

\[
X_2 = k X_1
\]

where $k \geq 1$. Then this bound becomes
