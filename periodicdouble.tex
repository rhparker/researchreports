...
Note that this will change with both $\lambda$ and with $X$, but the effect will not be multiplicative (since this will still happen if we take $\lambda = 0$. Thus this should be order $e^{-\alpha X} + |\lambda|$. These two things should (eventually) be of the same order, so that is ok.
\\
...
Luchkily, $p_2(X; \lambda)$ does not enter into the final bound, since we do not have a decay rate for it, but only know that its limit is 0 as $|X| \rightarrow \infty$.\\
...
This has two roots at 0 (which are expected) as well as two roots at $\pm \sqrt{-2a/M}$. This agrees with the leading-order versino for the exponentially weighted case. Again, these will be purely imaginary or real depending on the sign of $a$.
