...
&+ |c_1^-| + e^{\nu(\lambda)k X_1} |G||c_2^-| + (e^{-\tilde{\alpha} X_1} + |G|) |\lambda|^2 |d| ) ) \\
&= C( (e^{-\alpha X_1} + |G|) |b| + p_5(\lambda) |c_1^-| + |c_1^-| + (e^{-\tilde{\alpha} X_1} + |G|) |\lambda|^2 |d| \\
&+ (p_1(X_1; \lambda) + p_2(X_1; \lambda) + e^{\nu(\lambda) k X_1} |G|) \\
&\:\:\:\:\:( ( e^{-\alpha X_1} + |G|) |b| + |c_1^-| + (e^{-\tilde{\alpha} X_1} + |G|) |\lambda|^2 + |D_1|) |d| )\\
&+ e^{\nu(\lambda)k X_1} |G||c_2^-| \\
&\leq C( (e^{-\alpha X_1} + |G|) |b| + |c_1^-| + e^{\nu(\lambda)k X_1} |G||c_2^-| \\
...
Again, this one is not good enough. Compare to (3.39) in Sandstede (1998). The substitution of the $B_1$ bound (with the $e^{-|\nu(\lambda)| X_1} $ multiplier) for $|c^-|$ gives us a $|D_1|$ term by itself. We need this to be something of order $e^{-\alpha X_1}|D_1|$ to get the bound we need.
...
&= -\langle \Psi(0), \Phi^u_+(0, X_1; 0) P^u_0 D_1 d \rangle + \mathcal{O}( p_6(X_1; \lambda) e^{-\alpha X_1}|D_1||d| + e^{-\alpha X_1} (|\lambda|^2 + |D_1|) |d|) \\
&= -\langle \Psi(X_1), P^u_0 D_1 d \rangle + \mathcal{O}( e^{-\alpha X_1}( |\lambda|^2 + |D_1|)|d|)
\end{align*}
...
These should (hopefully) be okay.
